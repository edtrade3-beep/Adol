    ...
    const ARABIC_MUSIC_PLAYLISTS = [
      { name: "Shaimaa El Shayeb Playlist", url: "https://open.spotify.com/search/shaimaa%20el%20shayeb" },
      { name: "Shaimaa El Shayeb - Best Songs", url: "https://www.anghami.com/search/shaimaa%20el%20shayeb" },
      { name: "Shaimaa El Shayeb - Apple Music", url: "https://music.apple.com/us/search?term=shaimaa%20el%20shayeb" }
    ];
    ...
    const app = {
      ...
      scanner: {
        ...
        telegramToken: "8734264037:AAFsVsx5SX9FSSd-ypK2c1aa5vDt9OzbefY",
        telegramChatId: "2130889720",
        status: "Idle",
      ...
      adol: {
        apiUrl: "/api/adol",
        watchlistText: "TSLA,AMD,NVDA,META,AMZN",
        selectedTicker: "",
        institutionalSearch: "",
        riskMode: "balanced",
        sideMode: "both",
    ...
    const el = {
      ...
      newsHubList: $("newsHubList"), liquidityHubBody: $("liquidityHubBody"),
      ...
      scannerTelegramToken: $("scannerTelegramToken"), scannerTelegramChatId: $("scannerTelegramChatId"),
      scannerHealthBtn: $("scannerHealthBtn"), scannerRunBtn: $("scannerRunBtn"), scannerSaveTelegramBtn: $("scannerSaveTelegramBtn"), scannerTelegramTestBtn: $("scannerTelegramTestBtn"), scannerStatus: $("scannerStatus"), scannerOutput: $("scannerOutput"),
      gdTitle: $("gdTitle"), gdCategory: $("gdCategory"), gdBuyPrice: $("gdBuyPrice"), gdSoldPrice: $("gdSoldPrice"), gdShipping: $("gdShipping"), gdFeePct: $("gdFeePct"),
      gdCondition: $("gdCondition"), gdDemand: $("gdDemand"), gdRisk: $("gdRisk"), gdEase: $("gdEase"), gdLink: $("gdLink"), gdNotes: $("gdNotes"),
      gdCalcBtn: $("gdCalcBtn"), gdSaveBtn: $("gdSaveBtn"), gdSendAlertBtn: $("gdSendAlertBtn"), gdAutoAlert: $("gdAutoAlert"),
      gdStatus: $("gdStatus"), gdTotalCost: $("gdTotalCost"), gdFees: $("gdFees"), gdProfit: $("gdProfit"), gdRoi: $("gdRoi"), gdScore: $("gdScore"), gdGrade: $("gdGrade"), gdGradeBadge: $("gdGradeBadge"),
      gdTelegramToken: $("gdTelegramToken"), gdTelegramChatId: $("gdTelegramChatId"), gdSaveTelegramBtn: $("gdSaveTelegramBtn"), gdTestTelegramBtn: $("gdTestTelegramBtn"),
      gdPasteText: $("gdPasteText"), gdParseTextBtn: $("gdParseTextBtn"), gdCsvFile: $("gdCsvFile"), gdCsvImportBtn: $("gdCsvImportBtn"),
      gdSourceEbay: $("gdSourceEbay"), gdSourceCraigslist: $("gdSourceCraigslist"), gdSourceSlickdeals: $("gdSourceSlickdeals"), gdSourceRedditDeals: $("gdSourceRedditDeals"), gdSourceDealNews: $("gdSourceDealNews"), gdSourceDealslist: $("gdSourceDealslist"), gdSourceFacebook: $("gdSourceFacebook"), gdSourceSerp: $("gdSourceSerp"),
      gdScanQuery: $("gdScanQuery"), gdScanMinPrice: $("gdScanMinPrice"), gdScanMaxPrice: $("gdScanMaxPrice"), gdScanMinProfit: $("gdScanMinProfit"), gdScanDistance: $("gdScanDistance"), gdScanCity: $("gdScanCity"),
      gdRunScanBtn: $("gdRunScanBtn"), gdOpenSearchesBtn: $("gdOpenSearchesBtn"), gdImportTopBtn: $("gdImportTopBtn"), gdSendNewCandidatesBtn: $("gdSendNewCandidatesBtn"), gdCandidatesBody: $("gdCandidatesBody"), gdScanStatusBadge: $("gdScanStatusBadge"), gdAutoScanEvery5: $("gdAutoScanEvery5"), gdAutoSendOnScan: $("gdAutoSendOnScan"), gdAutoScanNext: $("gdAutoScanNext"),
      gdSearch: $("gdSearch"), gdFilterGrade: $("gdFilterGrade"), gdSortBy: $("gdSortBy"), gdSortDir: $("gdSortDir"), gdClearDealsBtn: $("gdClearDealsBtn"), gdCompSummary: $("gdCompSummary"), gdCompBody: $("gdCompBody"),
      gdDealsBody: $("gdDealsBody"), gdDealsCount: $("gdDealsCount"),
      adolWatchlistInput: $("adolWatchlistInput"), adolScanBtn: $("adolScanBtn"), adolRefreshBtn: $("adolRefreshBtn"), adolSessionBadge: $("adolSessionBadge"),
      adolMetaSession: $("adolMetaSession"), adolMetaMarket: $("adolMetaMarket"), adolMetaLast: $("adolMetaLast"), adolMetaNext: $("adolMetaNext"),
      adolPageTabs: $("adolPageTabs"), adolPageBody: $("adolPageBody"),
      ...
      arabicMusicQuery: $("arabicMusicQuery"), arabicMusicOpen: $("arabicMusicOpen"),
    ...
    const loginBypass = new URLSearchParams(window.location.search).get("login");
    if (loginBypass === "1" || loginBypass === "true") {
      if (el.loginShell) el.loginShell.classList.add("hidden");
    ...
    const ADOL_PAGES = [
      ...
      { id: "institutional", label: "Institutional Mode" },
      { id: "optionsflow", label: "Options Flow" },
      { id: "social", label: "Social Sentiment" },
      { id: "watchlist", label: "Watchlist" },
      { id: "report", label: "AI Market Report" },
      { id: "settings", label: "Settings" }
    ];

    let loginInProgress = false;
    let gdAutoScanTimer = null;
    let gdAutoScanBusy = false;
    let adolScanBusy = false;
    window.forcePlatformLogin = async function forcePlatformLogin() {
    ...
    function saveSettingsLocal() {
      localStorage.setItem("market-ai-platform-settings", JSON.stringify({
        ...
        scanner: app.scanner,
        greatDeals: app.greatDeals,
        adol: {
          apiUrl: app.adol.apiUrl,
          watchlistText: app.adol.watchlistText,
          selectedTicker: app.adol.selectedTicker,
          institutionalSearch: app.adol.institutionalSearch,
          riskMode: app.adol.riskMode,
          sideMode: app.adol.sideMode,
          newsCallFilter: app.adol.newsCallFilter,
          page: app.adol.page,
          table: app.adol.table
        },
    ...
    function loadSettingsLocal() {
      ...
      try {
        ...
        if (p.scanner && typeof p.scanner === "object") {
          ...
          if (typeof p.scanner.telegramToken === "string") app.scanner.telegramToken = p.scanner.telegramToken.trim();
          if (typeof p.scanner.telegramChatId === "string") app.scanner.telegramChatId = p.scanner.telegramChatId.trim();
        ...
        if (p.greatDeals && typeof p.greatDeals === "object") {
          if (typeof p.greatDeals.telegramToken === "string") app.greatDeals.telegramToken = p.greatDeals.telegramToken.trim() || app.greatDeals.telegramToken;
          if (typeof p.greatDeals.telegramChatId === "string") app.greatDeals.telegramChatId = p.greatDeals.telegramChatId.trim() || app.greatDeals.telegramChatId;
          if (typeof p.greatDeals.autoAlert === "boolean") app.greatDeals.autoAlert = p.greatDeals.autoAlert;
          if (p.greatDeals.form && typeof p.greatDeals.form === "object") app.greatDeals.form = { ...app.greatDeals.form, ...p.greatDeals.form };
          if (p.greatDeals.filters && typeof p.greatDeals.filters === "object") app.greatDeals.filters = { ...app.greatDeals.filters, ...p.greatDeals.filters };
          if (p.greatDeals.checklist && typeof p.greatDeals.checklist === "object") app.greatDeals.checklist = { ...app.greatDeals.checklist, ...p.greatDeals.checklist };
          if (p.greatDeals.hybrid && typeof p.greatDeals.hybrid === "object") {
            app.greatDeals.hybrid = { ...app.greatDeals.hybrid, ...p.greatDeals.hybrid };
            if (Array.isArray(p.greatDeals.hybrid.candidates)) app.greatDeals.hybrid.candidates = p.greatDeals.hybrid.candidates.slice(0, 200);
          }
          if (Array.isArray(p.greatDeals.deals)) app.greatDeals.deals = p.greatDeals.deals.slice(0, 500);
          if (typeof p.greatDeals.editingId === "string") app.greatDeals.editingId = p.greatDeals.editingId;
          if (p.greatDeals.lastCalc && typeof p.greatDeals.lastCalc === "object") app.greatDeals.lastCalc = p.greatDeals.lastCalc;
          if (typeof p.greatDeals.status === "string") app.greatDeals.status = p.greatDeals.status;
        }
        if (p.adol && typeof p.adol === "object") {
          if (typeof p.adol.apiUrl === "string" && p.adol.apiUrl.trim()) app.adol.apiUrl = p.adol.apiUrl.trim();
          if (typeof p.adol.watchlistText === "string") app.adol.watchlistText = p.adol.watchlistText.trim();
          if (typeof p.adol.selectedTicker === "string") app.adol.selectedTicker = p.adol.selectedTicker.trim().toUpperCase();
          if (typeof p.adol.institutionalSearch === "string") app.adol.institutionalSearch = p.adol.institutionalSearch.trim().toUpperCase();
          if (typeof p.adol.riskMode === "string" && ["aggressive", "balanced", "conservative"].includes(p.adol.riskMode)) app.adol.riskMode = p.adol.riskMode;
          if (typeof p.adol.sideMode === "string" && ["both", "long", "short"].includes(p.adol.sideMode)) app.adol.sideMode = p.adol.sideMode;
          if (typeof p.adol.newsCallFilter === "string" && ["ALL", "UPGRADE", "DOWNGRADE"].includes(String(p.adol.newsCallFilter).toUpperCase())) {
            app.adol.newsCallFilter = String(p.adol.newsCallFilter).toUpperCase();
          }
          if (typeof p.adol.page === "string" && p.adol.page.trim()) {
            const requestedPage = p.adol.page.trim();
            const exists = ADOL_PAGES.some((page) => page.id === requestedPage);
            app.adol.page = exists ? requestedPage : "liquidity";
          }
          if (p.adol.table && typeof p.adol.table === "object") {
            app.adol.table = { ...app.adol.table, ...p.adol.table };
          }
          app.adol.table.trend = "ALL";
          app.adol.table.action = "ALL";
          if (app.adol.table.minFinalAi === undefined || app.adol.table.minFinalAi === null) app.adol.table.minFinalAi = "";
        }
    ...
    function applyArabicMusicSource(url) {
      ...
      if (el.arabicMusicOpen) {
        el.arabicMusicOpen.setAttribute("href", pick.url);
        el.arabicMusicOpen.textContent = `Open: ${pick.name}`;
      }
    }
    ...
    function parseInstitutionalJson(rawText) {
      const text = String(rawText || "").trim();
      if (!text) return null;
      const candidates = [];
      candidates.push(text);
      const fenceMatch = text.match(/```(?:json)?\s*([\s\S]*?)```/i);
      if (fenceMatch && fenceMatch[1]) candidates.push(String(fenceMatch[1]).trim());
      ...
      for (const candidate of candidates) {
        try {
          ...
          if (parsed && typeof parsed === "object" && (parsed.overall_bias || parsed.setup_classification || parsed.action)) {
            return parsed;
          }
        } catch {}
      }
    ...
    function institutionalToneClass(colorTag, fallback = "warn") {
      const tag = String(colorTag || "").toUpperCase();
      if (tag === "GREEN") return "tone-ok";
      if (tag === "RED") return "tone-danger";
      if (tag === "ORANGE") return "tone-warn";
      if (tag === "YELLOW") return "tone-info";
      return fallback;
    }

    function renderInstitutionalJsonDashboard(target, payload) {
      const data = payload || {};
      const bias = data.overall_bias || {};
      const action = data.action || {};
      const heat = data.heat_risk || {};
      const targets = data.targets || {};
      const rr = data.risk_reward || {};
      const probs = data.probabilities || {};
      const inst = data.institutional_activity_analysis || {};
      const mom = data.momentum_strength || {};
      const levels = data.levels || {};
      const cardTone = institutionalToneClass(action.color_tag || bias.color_tag || heat.color_tag);

      target.innerHTML = `
        <div class="ai-report-grid">
          <article class="ai-report-card ${cardTone}">
            <div class="ai-report-title">Overall Bias</div>
            <div class="ai-report-body"><strong>${escapeHtml(String(bias.bias || "NEUTRAL"))}</strong> • Confidence ${escapeHtml(String(data.ai_confidence_score ?? 0))}/100<br>${escapeHtml(String(bias.summary || "UNAVAILABLE"))}</div>
          </article>
          <article class="ai-report-card ${institutionalToneClass(heat.color_tag, "tone-warn")}">
            <div class="ai-report-title">Setup + Heat</div>
            <div class="ai-report-body">${escapeHtml(String(data.setup_classification || "WAIT"))}<br>Heat: <strong>${escapeHtml(String(heat.state || "NEUTRAL"))}</strong> • Grade ${escapeHtml(String(data.trade_quality_grade || "C"))}</div>
          </article>
          <article class="ai-report-card ${institutionalToneClass(action.color_tag, "tone-info")}">
            <div class="ai-report-title">Action</div>
            <div class="ai-report-body"><strong>${escapeHtml(String(action.decision || "WAIT FOR PULLBACK"))}</strong><br>${escapeHtml(String(action.reason || "UNAVAILABLE"))}</div>
          </article>
          <article class="ai-report-card tone-ok">
            <div class="ai-report-title">Entry / Stop</div>
            <div class="ai-report-body">Entry: ${escapeHtml(String(data.entry_zone?.min ?? 0))} - ${escapeHtml(String(data.entry_zone?.max ?? 0))}<br>Stop: ${escapeHtml(String(data.stop_loss?.price ?? 0))} (${escapeHtml(String(data.stop_loss?.basis || "structure"))})</div>
          </article>
          <article class="ai-report-card tone-info">
            <div class="ai-report-title">Targets + R/R</div>
            <div class="ai-report-body">T1 ${escapeHtml(String(targets.t1 ?? 0))} • T2 ${escapeHtml(String(targets.t2 ?? 0))} • T3 ${escapeHtml(String(targets.t3 ?? 0))}<br>R/R: ${escapeHtml(String(rr.to_t1 ?? 0))} / ${escapeHtml(String(rr.to_t2 ?? 0))} / ${escapeHtml(String(rr.to_t3 ?? 0))}</div>
          </article>
          <article class="ai-report-card tone-warn">
            <div class="ai-report-title">Institutional Read</div>
            <div class="ai-report-body">Options: ${escapeHtml(String(inst.options_flow_bias || "UNAVAILABLE"))}<br>Dark pool: ${escapeHtml(String(inst.dark_pool_bias || "UNAVAILABLE"))}<br>Smart money: ${escapeHtml(String(inst.smart_money_accumulation ?? 0))}/100</div>
          </article>
          <article class="ai-report-card tone-info">
            <div class="ai-report-title">Momentum + Probabilities</div>
            <div class="ai-report-body">Momentum: ${escapeHtml(String(mom.state || "MODERATE"))} (${escapeHtml(String(mom.score ?? 0))}/100)<br>Continuation ${escapeHtml(String(probs.trend_continuation_probability ?? 0))}% • Fakeout ${escapeHtml(String(probs.fake_breakout_probability ?? 0))}% • Squeeze ${escapeHtml(String(probs.squeeze_potential ?? 0))}%</div>
          </article>
          <article class="ai-report-card tone-danger">
            <div class="ai-report-title">Risk Snapshot</div>
            <div class="ai-report-body">Trap ${escapeHtml(String(probs.trap_risk ?? 0))}% • Exhaustion ${escapeHtml(String(probs.exhaustion_risk ?? 0))}%<br>Volatility context included in invalidation.</div>
          </article>
          <article class="ai-report-card tone-warn">
            <div class="ai-report-title">Levels</div>
            <div class="ai-report-body">Support: ${escapeHtml((Array.isArray(levels.support) ? levels.support.join(", ") : "N/A"))}<br>Resistance: ${escapeHtml((Array.isArray(levels.resistance) ? levels.resistance.join(", ") : "N/A"))}<br>Liquidity: ${escapeHtml((Array.isArray(levels.liquidity_zones) ? levels.liquidity_zones.join(", ") : "N/A"))}</div>
          </article>
          <article class="ai-report-card tone-ok">
            <div class="ai-report-title">Execution Plan</div>
            <div class="ai-report-body">${escapeHtml(String(data.execution_plan?.one_line_plan || "UNAVAILABLE"))}<br><strong>Invalid if:</strong> ${escapeHtml(String(data.execution_plan?.invalidation_trigger || "UNAVAILABLE"))}</div>
          </article>
        </div>
      `;
    }
    ...
    function renderAiSections(target, rawText) {
      ...
      const institutional = parseInstitutionalJson(text);
      if (institutional) {
        renderInstitutionalJsonDashboard(target, institutional);
        return;
      }
    ...
    function normalizeScannerBaseUrl(value) {
      ...
      const fallback = `${window.location.protocol}//${host}:${port}`;
      if (!raw) return fallback;
      const withProto = /^https?:\/\//i.test(raw) ? raw : `http://${raw}`;
      try {
        const u = new URL(withProto);
        const isLocalHost = /^(127\.0\.0\.1|localhost)$/i.test(u.hostname);
        if (isLocalHost && String(u.port || "") === "3012") return fallback;
        return u.origin;
      } catch {
        return fallback;
      }
    ...
    function renderScanner() {
      ...
      if (el.scannerTelegramToken && !el.scannerTelegramToken.value) el.scannerTelegramToken.value = app.scanner.telegramToken || "";
      if (el.scannerTelegramChatId && !el.scannerTelegramChatId.value) el.scannerTelegramChatId.value = app.scanner.telegramChatId || "";
    ...
    async function scannerFetch(path, init = {}) {
      ...
      try {
        return await makeReq(base);
      } catch (error) {
        const fallbackBase = `${window.location.protocol}//${window.location.hostname}:${window.location.port || "3001"}`;
        if (fallbackBase !== base) {
          const retried = await makeReq(fallbackBase);
          app.scanner.apiUrl = fallbackBase;
          if (el.scannerApiUrl) el.scannerApiUrl.value = fallbackBase;
          return retried;
        }
        throw error;
      }
    ...
    async function saveScannerTelegramConfig() {
      app.scanner.apiUrl = normalizeScannerBaseUrl(el.scannerApiUrl?.value || app.scanner.apiUrl);
      const token = String(el.scannerTelegramToken?.value || "").trim();
      const chatId = String(el.scannerTelegramChatId?.value || "").trim();
      app.scanner.telegramToken = token;
      app.scanner.telegramChatId = chatId;
      if (!token && !chatId) {
        app.scanner.status = "Missing Telegram values";
        app.scanner.output = "Enter Telegram Bot Token and Telegram Chat ID, then click Save Telegram.";
        saveSettingsLocal();
        renderScanner();
        return;
      }
      app.scanner.status = "Saving Telegram config...";
      renderScanner();
      try {
        const data = await scannerFetch("/set-telegram-config", {
          method: "POST",
          body: JSON.stringify({ telegram_bot_token: token, telegram_chat_id: chatId })
        });
        app.scanner.status = data?.telegram_configured ? "Telegram configured" : "Telegram not configured";
        app.scanner.output = JSON.stringify(data, null, 2);
        pushLog("Scanner Telegram config saved");
      } catch (error) {
        app.scanner.status = "Save failed";
        app.scanner.output = `Telegram config save failed:\n${error?.message || "Unknown error"}`;
      }
      saveSettingsLocal();
      renderScanner();
    }

    async function testScannerTelegram() {
      app.scanner.apiUrl = normalizeScannerBaseUrl(el.scannerApiUrl?.value || app.scanner.apiUrl);
      app.scanner.status = "Testing Telegram...";
      renderScanner();
      try {
        const data = await scannerFetch("/test-telegram", { method: "POST", body: JSON.stringify({}) });
        const statusCode = data?.result?.status_code ? ` (HTTP ${data.result.status_code})` : "";
        app.scanner.status = "Telegram test sent";
        app.scanner.output = `Telegram test successful${statusCode}\n\n` + JSON.stringify(data, null, 2);
        pushLog("Scanner Telegram test sent");
      } catch (error) {
        app.scanner.status = "Telegram test failed";
        app.scanner.output = `Telegram test failed:\n${error?.message || "Unknown error"}\n\nCheck TELEGRAM_BOT_TOKEN and TELEGRAM_CHAT_ID in your scanner service environment.`;
      }
      saveSettingsLocal();
      renderScanner();
    }

    function gdNum(value) {
      if (value === null || value === undefined) return 0;
      const n = Number(String(value).replace(/[^0-9.\-]/g, ""));
      return Number.isFinite(n) ? n : 0;
    }

    function gdClamp(value, min, max) {
      return Math.max(min, Math.min(max, value));
    }

    function gdConditionScore(condition) {
      const map = { new: 100, excellent: 90, good: 76, fair: 58, parts: 20 };
      return map[String(condition || "good").toLowerCase()] || 60;
    }

    function gdGrade(profit, roi, riskScore) {
      if (profit > 150 && roi > 50 && riskScore <= 3) return "A+";
      if (profit > 100 && roi > 35 && riskScore <= 4) return "A";
      if (profit > 50 && roi > 25) return "B";
      if (profit > 20 && roi > 10) return "C";
      return "Reject";
    }

    function gdDealScore(input, calc) {
      const marginPct = calc.soldPrice > 0 ? (calc.netProfit / calc.soldPrice) * 100 : 0;
      const profitMarginScore = gdClamp(marginPct * 2, 0, 100);
      const roiScore = gdClamp(calc.roi * 1.4, 0, 100);
      const demandScore = gdClamp((gdNum(input.demand) / 5) * 100, 0, 100);
      const condScore = gdConditionScore(input.condition);
      const riskScoreNormalized = gdClamp((6 - gdNum(input.risk)) * 20, 0, 100);
      const easeScore = gdClamp((gdNum(input.ease) / 5) * 100, 0, 100);
      const weighted = (
        profitMarginScore * 0.2 +
        roiScore * 0.2 +
        demandScore * 0.15 +
        condScore * 0.15 +
        riskScoreNormalized * 0.15 +
        easeScore * 0.15
      );
      return Math.round(gdClamp(weighted, 0, 100));
    }

    function gdBuildInputFromForm() {
      return {
        title: String(el.gdTitle?.value || "").trim(),
        category: String(el.gdCategory?.value || "Other").trim(),
        buyPrice: gdNum(el.gdBuyPrice?.value),
        soldPrice: gdNum(el.gdSoldPrice?.value),
        shippingCost: gdNum(el.gdShipping?.value),
        feePct: gdNum(el.gdFeePct?.value),
        condition: String(el.gdCondition?.value || "good").trim().toLowerCase(),
        demand: gdNum(el.gdDemand?.value || 3),
        risk: gdNum(el.gdRisk?.value || 3),
        ease: gdNum(el.gdEase?.value || 3),
        link: String(el.gdLink?.value || "").trim(),
        notes: String(el.gdNotes?.value || "").trim()
      };
    }

    function gdCompute(input) {
      const totalCost = input.buyPrice + input.shippingCost;
      const estimatedFees = input.soldPrice * (input.feePct / 100);
      const netProfit = input.soldPrice - estimatedFees - totalCost;
      const roi = totalCost > 0 ? (netProfit / totalCost) * 100 : 0;
      const score = gdDealScore(input, { soldPrice: input.soldPrice, netProfit, roi });
      const grade = gdGrade(netProfit, roi, input.risk);
      const reason = grade === "A+" ? "High-margin flip with strong ROI."
        : grade === "A" ? "Strong flip profile with solid upside."
        : grade === "B" ? "Decent flip if negotiation is clean."
        : grade === "C" ? "Thin edge. Improve buy price."
        : "Low edge or high risk.";
      return {
        totalCost,
        estimatedFees,
        netProfit,
    ...
    function gdToneClassFromGrade(grade) {
    ...
    function gdTelegramMessage(deal, calc) {
      return [
        "?? FLIP DEAL FOUND",
        "",
        `Item: ${deal.title || "-"}`,
        `Buy Price: ${formatUsd(deal.buyPrice)}`,
        `Estimated Sold Price: ${formatUsd(deal.soldPrice)}`,
        `Net Profit: ${formatUsd(calc.netProfit)}`,
        `ROI: ${formatPctSigned(calc.roi)}`,
        `Grade: ${calc.grade}`,
        `Risk: ${gdRiskLabel(deal.risk)}`,
        `Link: ${deal.link || "-"}`
      ].join("\n");
    }

    async function gdSendTelegram(text) {
      const token = String(el.gdTelegramToken?.value || app.greatDeals.telegramToken || "").trim();
      const chatId = String(el.gdTelegramChatId?.value || app.greatDeals.telegramChatId || "").trim();
    ...
    function gdSetStatus(message) {
      app.greatDeals.status = String(message || "Ready.");
      if (el.gdStatus) el.gdStatus.textContent = app.greatDeals.status;
    }

    function gdRenderKpis(calc) {
      const c = calc || app.greatDeals.lastCalc || { totalCost: 0, estimatedFees: 0, netProfit: 0, roi: 0, score: 0, grade: "Reject" };
      if (el.gdTotalCost) el.gdTotalCost.textContent = formatUsd(c.totalCost);
      if (el.gdFees) el.gdFees.textContent = formatUsd(c.estimatedFees);
      if (el.gdProfit) el.gdProfit.textContent = formatUsd(c.netProfit);
      if (el.gdRoi) el.gdRoi.textContent = formatPctSigned(c.roi);
      if (el.gdScore) el.gdScore.textContent = String(Math.round(c.score || 0));
      if (el.gdGrade) el.gdGrade.textContent = String(c.grade || "Reject");
      if (el.gdGradeBadge) el.gdGradeBadge.textContent = `Grade ${String(c.grade || "--")}`;

      const kpis = [
        [el.gdProfit?.closest(".gd-kpi"), c.netProfit],
        [el.gdRoi?.closest(".gd-kpi"), c.roi],
        [el.gdScore?.closest(".gd-kpi"), c.score],
        [el.gdGrade?.closest(".gd-kpi"), c.grade]
      ];
      kpis.forEach(([node]) => {
        if (!node) return;
        node.classList.remove("good", "warn", "bad", "cool");
      });
      const tone = gdToneClassFromGrade(c.grade);
      if (el.gdProfit?.closest(".gd-kpi")) el.gdProfit.closest(".gd-kpi").classList.add(tone);
      if (el.gdRoi?.closest(".gd-kpi")) el.gdRoi.closest(".gd-kpi").classList.add(tone);
      if (el.gdScore?.closest(".gd-kpi")) el.gdScore.closest(".gd-kpi").classList.add(tone);
      if (el.gdGrade?.closest(".gd-kpi")) el.gdGrade.closest(".gd-kpi").classList.add(tone);
    }

    function gdSyncFormFromState() {
      const form = app.greatDeals.form || {};
      const setIfIdle = (node, value) => {
        if (!node) return;
        if (document.activeElement === node) return;
        node.value = value;
      };
      setIfIdle(el.gdTitle, form.title || "");
      setIfIdle(el.gdCategory, form.category || "Other");
      setIfIdle(el.gdBuyPrice, form.buyPrice || "");
      setIfIdle(el.gdSoldPrice, form.soldPrice || "");
      setIfIdle(el.gdShipping, form.shippingCost || "0");
      setIfIdle(el.gdFeePct, form.feePct || "13");
      setIfIdle(el.gdCondition, form.condition || "good");
      setIfIdle(el.gdDemand, String(form.demand || "3"));
      setIfIdle(el.gdRisk, String(form.risk || "3"));
      setIfIdle(el.gdEase, String(form.ease || "3"));
      setIfIdle(el.gdLink, form.link || "");
      setIfIdle(el.gdNotes, form.notes || "");
      setIfIdle(el.gdTelegramToken, app.greatDeals.telegramToken || "");
      setIfIdle(el.gdTelegramChatId, app.greatDeals.telegramChatId || "");
      if (el.gdAutoAlert) el.gdAutoAlert.checked = !!app.greatDeals.autoAlert;
      setIfIdle(el.gdSearch, app.greatDeals.filters.search || "");
      setIfIdle(el.gdFilterGrade, app.greatDeals.filters.grade || "ALL");
      setIfIdle(el.gdSortBy, app.greatDeals.filters.sortBy || "profit");
      setIfIdle(el.gdSortDir, app.greatDeals.filters.sortDir || "desc");
      const checks = app.greatDeals.checklist || {};
      const nodes = Array.from(document.querySelectorAll(".gdChecklistItem"));
      if (nodes[0]) nodes[0].checked = !!checks.comps;
      if (nodes[1]) nodes[1].checked = !!checks.condition;
      if (nodes[2]) nodes[2].checked = !!checks.shipping;
      if (nodes[3]) nodes[3].checked = !!checks.risk;
      if (nodes[4]) nodes[4].checked = !!checks.negotiate;
      if (nodes[5]) nodes[5].checked = !!checks.pickup;
      const hybrid = app.greatDeals.hybrid || {};
      setIfIdle(el.gdScanQuery, hybrid.query || "");
      setIfIdle(el.gdScanMinPrice, String(hybrid.minPrice ?? 20));
      setIfIdle(el.gdScanMaxPrice, String(hybrid.maxPrice ?? 3000));
      setIfIdle(el.gdScanMinProfit, String(hybrid.minProfit ?? 0));
      setIfIdle(el.gdScanDistance, String(hybrid.distance ?? 0));
      setIfIdle(el.gdScanCity, hybrid.city || "cincinnati");
      if (el.gdSourceEbay) el.gdSourceEbay.checked = hybrid.sourceEbay !== false;
      if (el.gdSourceCraigslist) el.gdSourceCraigslist.checked = hybrid.sourceCraigslist !== false;
      if (el.gdSourceFacebook) el.gdSourceFacebook.checked = hybrid.sourceFacebook !== false;
      if (el.gdSourceSerp) el.gdSourceSerp.checked = hybrid.sourceSerp !== false;
      gdHybridSetStatus(hybrid.status || "Manual + Public Feeds");
      gdRenderAutoScanMeta();
      gdSetStatus(app.greatDeals.status || "Ready.");
      gdRenderKpis(app.greatDeals.lastCalc);
    }

    function gdNormalizeDealRow(row = {}) {
      const out = {
        ...row,
        id: String(row.id || `deal_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`),
        ...
        buyPrice: gdNum(row.askingPrice ?? row.buyPrice ?? 0),
        soldPrice: gdNum(row.marketValue ?? row.soldPrice ?? 0),
        shippingCost: gdNum(row.shippingCost ?? row.shipping_cost ?? 0),
        feePct: gdNum(row.feePct ?? row.fee_pct ?? 13),
        demand: gdNum(row.demand ?? 3) || 3,
        risk: gdNum(row.riskPenalty ?? row.risk ?? 0),
        ease: gdNum(row.ease ?? 3) || 3,
        profit: gdNum(row.profit ?? row.netProfit ?? 0),
        roi: gdNum(row.margin ?? row.roi ?? 0),
        score: gdNum(row.score ?? 0),
        grade: String(row.grade || "Skip"),
        reason: String(row.reason || row.explanation || ""),
        createdAt: gdNum(row.createdAt ?? row.created_at ?? Date.now())
      };
      if (!Number.isFinite(out.createdAt) || out.createdAt <= 0) out.createdAt = Date.now();
      return out;
    }

    function gdSortAndFilterDeals() {
      const state = app.greatDeals.filters;
      const search = String(state.search || "").toLowerCase();
      const grade = String(state.grade || "ALL").toUpperCase();
      const sortBy = String(state.sortBy || "profit");
      const sortDir = String(state.sortDir || "desc") === "asc" ? 1 : -1;
      let rows = Array.isArray(app.greatDeals.deals) ? [...app.greatDeals.deals] : [];
      if (search) {
        rows = rows.filter((d) => {
          const blob = [d.title, d.category, d.notes, d.link].join(" ").toLowerCase();
          return blob.includes(search);
        });
      }
      if (grade !== "ALL") rows = rows.filter((d) => String(d.grade || "").toUpperCase() === grade);
      rows.sort((a, b) => {
        const av = sortBy === "createdAt" ? gdNum(a.createdAt) : gdNum(a[sortBy]);
        const bv = sortBy === "createdAt" ? gdNum(b.createdAt) : gdNum(b[sortBy]);
        if (av === bv) return 0;
    ...
    function gdRenderDealsTable() {
      if (!el.gdDealsBody) return;
      const rows = gdSortAndFilterDeals();
      if (!rows.length) {
        el.gdDealsBody.innerHTML = `<tr><td colspan="12" style="text-align:center;color:var(--muted);">No deals saved yet.</td></tr>`;
        if (el.gdDealsCount) el.gdDealsCount.textContent = "0 deals";
        return;
      }
      el.gdDealsBody.innerHTML = rows.map((d) => `
        <tr data-id="${escapeHtml(String(d.id || ""))}">
          <td>${escapeHtml(String(d.title || "-"))}</td>
          <td>${escapeHtml(String(d.category || "-"))}</td>
          <td><span class="adol-tone info">${escapeHtml(String(d.status || "new"))}</span></td>
          <td>${formatUsd(d.buyPrice)}</td>
          <td>${formatUsd(d.soldPrice)}</td>
          <td>${formatUsd(d.profit)}</td>
          <td>${formatPctSigned(d.roi)}</td>
          <td>${escapeHtml(String(Math.round(gdNum(d.score))))}</td>
          <td><span class="adol-tone ${gdToneClassFromGrade(d.grade)}">${escapeHtml(String(d.grade || "Reject"))}</span></td>
          <td>${escapeHtml(gdRiskLabel(d.risk))}</td>
          <td>${d.link ? `<a href="${escapeHtml(d.link)}" target="_blank" rel="noopener noreferrer">Open</a>` : "-"}</td>
          <td>
            <div class="gd-row-actions">
              <button class="gd-mini-btn" data-gd-action="edit" data-gd-id="${escapeHtml(String(d.id || ""))}">Edit</button>
              <button class="gd-mini-btn" data-gd-action="alert" data-gd-id="${escapeHtml(String(d.id || ""))}">Alert</button>
              <button class="gd-mini-btn" data-gd-action="contacted" data-gd-id="${escapeHtml(String(d.id || ""))}">Contacted</button>
              <button class="gd-mini-btn" data-gd-action="bought" data-gd-id="${escapeHtml(String(d.id || ""))}">Bought</button>
              <button class="gd-mini-btn" data-gd-action="sold" data-gd-id="${escapeHtml(String(d.id || ""))}">Sold</button>
              <button class="gd-mini-btn" data-gd-action="skip" data-gd-id="${escapeHtml(String(d.id || ""))}">Skip</button>
              <button class="gd-mini-btn" data-gd-action="delete" data-gd-id="${escapeHtml(String(d.id || ""))}">Delete</button>
            </div>
          </td>
        </tr>
      `).join("");
      if (el.gdDealsCount) el.gdDealsCount.textContent = `${rows.length} deals`;
    }
    ...
    async function gdLoadDealsFromBackend() {
      try {
        const qs = new URLSearchParams({
          search: String(app.greatDeals.filters.search || ""),
          grade: String(app.greatDeals.filters.grade || "ALL"),
          sortBy: String(app.greatDeals.filters.sortBy || "created_at"),
          sortDir: String(app.greatDeals.filters.sortDir || "desc")
        });
        const res = await fetch(`/api/great-deals/deals?${qs.toString()}`);
        const data = await res.json().catch(() => ({}));
        if (!res.ok || data?.ok === false) return;
        if (Array.isArray(data?.rows)) {
          app.greatDeals.deals = data.rows.map((row) => gdNormalizeDealRow(row));
          gdRenderDealsTable();
        }
      } catch {
        // local mode fallback
    ...
    function gdInferCategory(text) {
      const t = String(text || "").toLowerCase();
      if (/(iphone|ios|apple phone)/.test(t)) return "iPhones";
      if (/(macbook|imac|mac mini|apple laptop)/.test(t)) return "MacBooks";
      if (/(ps5|ps4|xbox|switch|console)/.test(t)) return "Game consoles";
      if (/(dewalt|milwaukee|drill|saw|tool)/.test(t)) return "Tools";
      if (/(sofa|couch|table|dresser|furniture|chair)/.test(t)) return "Furniture";
      if (/(washer|dryer|fridge|stove|dishwasher|appliance)/.test(t)) return "Appliances";
      if (/(sneaker|nike|jordan|adidas|yeezy)/.test(t)) return "Sneakers";
      if (/(rolex|omega|watch|seiko)/.test(t)) return "Watches";
      if (/(camera|canon|nikon|sony a7|dslr)/.test(t)) return "Cameras";
      if (/(tv|laptop|tablet|gpu|monitor|electronic)/.test(t)) return "Electronics";
      return "Other";
    }

    function gdInferCondition(text) {
      const t = String(text || "").toLowerCase();
      if (/(brand new|new in box|sealed|unused)/.test(t)) return "new";
      if (/(excellent|mint|like new)/.test(t)) return "excellent";
      if (/(fair|used fair|wear|scratches)/.test(t)) return "fair";
      if (/(parts|broken|not working|as-is)/.test(t)) return "parts";
      return "good";
    }

    function gdParseTextScanner() {
      const text = String(el.gdPasteText?.value || "").trim();
      if (!text) {
        gdSetStatus("Paste listing text first.");
        return;
      }
      const lines = text.split(/\r?\n/).map((l) => l.trim()).filter(Boolean);
      const firstLine = lines[0] || "";
      const priceMatch = text.match(/(?:\$|USD\s*)\s*([0-9]{1,3}(?:,[0-9]{3})*(?:\.[0-9]{1,2})?)/i);
      const linkMatch = text.match(/https?:\/\/[^\s)]+/i);
      app.greatDeals.form.title = firstLine || app.greatDeals.form.title;
      app.greatDeals.form.buyPrice = priceMatch ? String(gdNum(priceMatch[1])) : app.greatDeals.form.buyPrice;
      app.greatDeals.form.link = linkMatch ? linkMatch[0] : app.greatDeals.form.link;
      app.greatDeals.form.condition = gdInferCondition(text);
      app.greatDeals.form.category = gdInferCategory(text);
      const trimmed = text.length > 400 ? `${text.slice(0, 400)}...` : text;
      app.greatDeals.form.notes = app.greatDeals.form.notes ? `${app.greatDeals.form.notes}\n${trimmed}` : trimmed;
      gdSyncFormFromState();
      saveSettingsLocal();
      gdSetStatus("Extracted title/price/condition/link. Enter estimated sold price next.");
    }

    function gdGuessPriceFromText(text) {
      const match = String(text || "").match(/(?:\$|USD\s*)\s*([0-9]{1,3}(?:,[0-9]{3})*(?:\.[0-9]{1,2})?)/i);
      if (!match) return 0;
      return gdNum(match[1]);
    }

    function gdHybridSetStatus(text) {
      app.greatDeals.hybrid.status = String(text || "Manual + Public Feeds");
      if (el.gdScanStatusBadge) el.gdScanStatusBadge.textContent = app.greatDeals.hybrid.status;
    }

    function gdCandidateKey(candidate) {
      return `${String(candidate?.source || "").toLowerCase()}|${String(candidate?.title || "").toLowerCase()}|${String(candidate?.link || "").toLowerCase()}`;
    }

    function gdCandidateTelegramMessage(candidate) {
      const p = gdNum(candidate?.askingPrice);
      const mv = gdNum(candidate?.marketValue);
      const profit = gdNum(candidate?.profit);
      return [
        "?? GREAT DEAL CANDIDATE",
        "",
        `Source: ${String(candidate?.source || "-")}`,
        `Item: ${String(candidate?.title || "-")}`,
        `Asking: ${p > 0 ? formatUsd(p) : "-"}`,
        `Market Value: ${mv > 0 ? formatUsd(mv) : "-"}`,
        `Profit: ${profit ? formatUsd(profit) : "-"}`,
        `Grade: ${String(candidate?.grade || "Skip")}`,
        `Condition: ${String(candidate?.condition || "unknown")}`,
        `Link: ${String(candidate?.link || "-")}`
      ].join("\n");
    }

    function gdApplyAutoPolicyDefaults() {
      if (!app?.greatDeals?.hybrid) return;
      app.greatDeals.hybrid.autoScanEnabled = true;
      app.greatDeals.hybrid.autoScanIntervalMin = 5;
      app.greatDeals.hybrid.autoSendOnScan = true;
    }

    function gdRenderAutoScanMeta() {
      const hybrid = app.greatDeals.hybrid || {};
      if (el.gdAutoScanEvery5) el.gdAutoScanEvery5.checked = hybrid.autoScanEnabled !== false;
      if (el.gdAutoSendOnScan) el.gdAutoSendOnScan.checked = hybrid.autoSendOnScan !== false;
      if (!el.gdAutoScanNext) return;
      const intervalMs = 5 * 60000;
      const lastMs = gdNum(hybrid.lastAutoRunAt || 0);
      if (!hybrid.autoScanEnabled) {
        el.gdAutoScanNext.textContent = "Auto scan: Off";
        return;
      }
      if (!lastMs) {
        el.gdAutoScanNext.textContent = "Next auto scan: in <5m";
        return;
      }
      const nextMs = lastMs + intervalMs;
      const remain = Math.max(0, Math.ceil((nextMs - Date.now()) / 1000));
      if (remain <= 0) {
        el.gdAutoScanNext.textContent = "Next auto scan: now";
      } else {
        const mm = Math.floor(remain / 60);
        const ss = remain % 60;
        el.gdAutoScanNext.textContent = `Next auto scan: ${mm}:${String(ss).padStart(2, "0")}`;
      }
    }

    async function gdSendCandidatesToTelegram(candidates, limit = 5) {
      const rows = Array.isArray(candidates) ? candidates : [];
      if (!rows.length) return { sent: 0, skipped: 0 };
      const known = new Set(Array.isArray(app.greatDeals.hybrid.lastSentKeys) ? app.greatDeals.hybrid.lastSentKeys : []);
      const fresh = rows
        .filter((c) => ["A+", "A"].includes(String(c?.grade || "").toUpperCase()))
        .filter((c) => !known.has(gdCandidateKey(c)));
      if (!fresh.length) return { sent: 0, skipped: rows.length };

      const batch = fresh.slice(0, Math.max(1, limit));
      let sent = 0;
      for (const item of batch) {
        try {
          await gdSendTelegram(gdCandidateTelegramMessage(item));
          known.add(gdCandidateKey(item));
          sent += 1;
        } catch {
          // continue batch and report final sent count
        }
      }
      app.greatDeals.hybrid.lastSentKeys = Array.from(known).slice(-1000);
      saveSettingsLocal();
      return { sent, skipped: rows.length - sent };
    }

    function gdStartAutoScanScheduler() {
      if (gdAutoScanTimer) clearInterval(gdAutoScanTimer);
      gdAutoScanTimer = setInterval(async () => {
        try {
          const hybrid = app.greatDeals.hybrid || {};
          gdRenderAutoScanMeta();
          if (!hybrid.autoScanEnabled) return;
          if (gdAutoScanBusy) return;
          const query = String(hybrid.query || "").trim();
          if (!query) return;
          const intervalMs = 5 * 60000;
          const lastMs = gdNum(hybrid.lastAutoRunAt || 0);
          if (Date.now() - lastMs < intervalMs) return;
          gdAutoScanBusy = true;
          await gdRunHybridScan({ auto: true });
        } catch (error) {
          gdSetStatus(`Auto scan error: ${error?.message || "Unknown error"}`);
        } finally {
          gdAutoScanBusy = false;
          gdRenderAutoScanMeta();
        }
      }, 15000);
      gdRenderAutoScanMeta();
    ...
    async function gdFetchRssXml(feedUrl) {
      const localProxy = `/api/great-deals/rss-proxy?url=${encodeURIComponent(feedUrl)}`;
      let localErrMsg = "";
      try {
        ...
        if (res.ok) {
          ...
          throw new Error("Empty proxy XML");
        }
        const body = await res.text().catch(() => "");
        localErrMsg = `local ${res.status}`;
        if (body) {
          const m = body.match(/"error"\s*:\s*"([^"]+)"/i);
          if (m && m[1]) localErrMsg += ` ${m[1]}`;
        }
        throw new Error(localErrMsg);
      } catch (localErr) {
        const proxyUrl = `https://api.allorigins.win/raw?url=${encodeURIComponent(feedUrl)}`;
        const response = await fetch(proxyUrl, { cache: "no-store" });
        if (!response.ok) {
          throw new Error(`failed (${localErrMsg || "local blocked"}, fallback ${response.status})`);
        }
    ...
    function gdParseRssItems(xmlText, source, minPrice, maxPrice) {
      ...
      for (const item of items) {
        ...
        const desc = String(item.querySelector("description")?.textContent || "").trim();
        if (!title || !link) continue;
        const price = gdGuessPriceFromText(`${title} ${desc}`);
        if (minPrice > 0 && price > 0 && price < minPrice) continue;
        if (maxPrice > 0 && price > 0 && price > maxPrice) continue;
        const condition = gdInferCondition(`${title} ${desc}`);
        out.push({
          ...
          price,
          condition,
          notes: desc.replace(/<[^>]+>/g, " ").replace(/\s+/g, " ").trim().slice(0, 380)
        });
      }
      return out;
    }

    function gdRenderCandidates() {
      if (!el.gdCandidatesBody) return;
      const rows = Array.isArray(app.greatDeals.hybrid.candidates) ? app.greatDeals.hybrid.candidates : [];
      if (!rows.length) {
        el.gdCandidatesBody.innerHTML = `<tr><td colspan="10" style="text-align:center;color:var(--muted);">No candidates yet. Run Deal Spotter Scan.</td></tr>`;
        if (el.gdCompSummary) el.gdCompSummary.textContent = "No scan yet";
        if (el.gdCompBody) el.gdCompBody.textContent = "Run Deal Spotter scan to load comp explanations and top reasons.";
        return;
      }
      const best = rows
        .slice()
        .sort((a, b) => gdNum(b.score) - gdNum(a.score))
        .slice(0, 5)
        .map((x, idx) => `${idx + 1}. ${x.title} | ${x.grade} | Profit ${formatUsd(x.profit)} | Score ${Math.round(gdNum(x.score))}`)
        .join("\n");
      if (el.gdCompSummary) el.gdCompSummary.textContent = `${rows.length} scored candidates`;
      if (el.gdCompBody) el.gdCompBody.textContent = best || "No comp summary available.";
      el.gdCandidatesBody.innerHTML = rows.map((c) => `
        <tr>
          <td>${c.imageUrl ? `<img src="${escapeHtml(String(c.imageUrl))}" alt="item" style="width:48px;height:48px;object-fit:cover;border-radius:8px;border:1px solid var(--line);" />` : "-"}</td>
          <td>${escapeHtml(String(c.source || "-"))}</td>
          <td class="gd-candidates-title">${escapeHtml(String(c.title || "-"))}</td>
          <td>${gdNum(c.askingPrice) > 0 ? formatUsd(c.askingPrice) : "-"}</td>
          <td>${gdNum(c.marketValue) > 0 ? formatUsd(c.marketValue) : "-"}</td>
          <td>${formatUsd(gdNum(c.profit))}</td>
          <td><span class="adol-tone ${gdToneClassFromGrade(c.grade)}">${escapeHtml(String(c.grade || "Skip"))}</span></td>
          <td>${escapeHtml(String(c.condition || "good"))}</td>
          <td><a href="${escapeHtml(String(c.link || "#"))}" target="_blank" rel="noopener noreferrer">Open</a></td>
          <td>
            <div class="gd-row-actions">
              <button class="gd-mini-btn" type="button" data-gd-cand-import="${escapeHtml(String(c.id || ""))}">Import</button>
              <button class="gd-mini-btn" type="button" data-gd-cand-save="${escapeHtml(String(c.id || ""))}">Save</button>
            </div>
          </td>
        </tr>
      `).join("");
    ...
    function gdImportCandidate(id) {
      const c = (app.greatDeals.hybrid.candidates || []).find((x) => String(x.id) === String(id));
      if (!c) return;
      app.greatDeals.form.title = c.title || "";
      app.greatDeals.form.category = gdInferCategory(c.title || c.notes || "");
      app.greatDeals.form.buyPrice = gdNum(c.askingPrice) > 0 ? String(gdNum(c.askingPrice)) : "";
      app.greatDeals.form.soldPrice = gdNum(c.marketValue) > 0 ? String(gdNum(c.marketValue)) : app.greatDeals.form.soldPrice;
      app.greatDeals.form.condition = c.condition || "good";
      app.greatDeals.form.link = c.link || "";
      app.greatDeals.form.notes = [app.greatDeals.form.notes, `[${c.source}] ${c.notes || ""}`].filter(Boolean).join("\n");
      gdSyncFormFromState();
      saveSettingsLocal();
      gdSetStatus(`Candidate imported: ${c.title}`);
    }

    async function gdSaveCandidate(id, status = "new") {
      const c = (app.greatDeals.hybrid.candidates || []).find((x) => String(x.id) === String(id));
      if (!c) {
        gdSetStatus("Candidate not found.");
        return;
      }
      const payload = {
        id: String(c.id || `deal_${Date.now()}`),
        title: c.title,
        category: gdInferCategory(c.title || c.notes || ""),
        source: c.source || "Scanner",
        condition: c.condition || "good",
        link: c.link || "",
        imageUrl: c.imageUrl || "",
        notes: c.notes || "",
        askingPrice: gdNum(c.askingPrice),
        marketValue: gdNum(c.marketValue),
        feePct: gdNum(el.gdFeePct?.value || 13),
        repairCost: 0,
        pickupCost: 0,
        riskPenalty: 0,
        status
      ...
      try {
        const res = await fetch("/api/great-deals/deals", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(payload)
        });
        ...
        if (!res.ok || data?.ok === false) {
          gdSetStatus(`Save failed: ${data?.error || `HTTP ${res.status}`}`);
          return;
        }
        const deal = data?.deal ? gdNormalizeDealRow(data.deal) : null;
        if (deal) {
          const idx = app.greatDeals.deals.findIndex((d) => String(d.id) === String(deal.id));
          if (idx >= 0) app.greatDeals.deals[idx] = deal;
          else app.greatDeals.deals.unshift(deal);
          gdRenderDealsTable();
        }
        gdSetStatus(`Saved candidate (${status}): ${c.title}`);
      } catch (error) {
        gdSetStatus(`Save failed: ${error?.message || "Unknown error"}`);
      }
    }

    function gdParseCsvRows(text) {
      ...
      for (let i = 1; i < lines.length; i += 1) {
        ...
        rows.push({
          ...
          source: "CSV Upload",
          title: title,
          askingPrice: gdNum(row.asking_price || row.price || row.buy_price || 0),
          marketValue: gdNum(row.market_value || row.sold_price || row.est_sold_price || 0),
          ...
          grade: row.grade || "",
          score: gdNum(row.score || 0),
          profit: gdNum(row.profit || 0)
        });
      }
      return rows;
    }

    async function gdImportCsvFile() {
      const file = el.gdCsvFile?.files?.[0];
      if (!file) {
        gdSetStatus("Choose a CSV file first.");
        return;
      }
      const text = await file.text();
      const rows = gdParseCsvRows(text);
      if (!rows.length) {
        gdSetStatus("No valid listing rows found in CSV.");
        return;
      }
      const existing = Array.isArray(app.greatDeals.hybrid.candidates) ? app.greatDeals.hybrid.candidates : [];
      const combined = [...rows, ...existing];
      const seen = new Set();
      app.greatDeals.hybrid.candidates = combined.filter((row) => {
      ...
      gdRenderCandidates();
      saveSettingsLocal();
      gdSetStatus(`Imported ${rows.length} CSV listing(s).`);
    }

    async function gdRunHybridScan(options = {}) {
      const auto = !!options?.auto;
      const query = String(el.gdScanQuery?.value || app.greatDeals.hybrid.query || "").trim();
      const minPrice = gdNum(el.gdScanMinPrice?.value || app.greatDeals.hybrid.minPrice || 0);
      const maxPrice = gdNum(el.gdScanMaxPrice?.value || app.greatDeals.hybrid.maxPrice || 0);
      const minProfit = gdNum(el.gdScanMinProfit?.value || app.greatDeals.hybrid.minProfit || 0);
      const distance = gdNum(el.gdScanDistance?.value || app.greatDeals.hybrid.distance || 0);
      const city = String(el.gdScanCity?.value || app.greatDeals.hybrid.city || "cincinnati").trim().toLowerCase();
      const sourceEbay = !!el.gdSourceEbay?.checked;
      const sourceCraigslist = !!el.gdSourceCraigslist?.checked;
      const sourceFacebook = !!el.gdSourceFacebook?.checked;
      const sourceSerp = !!el.gdSourceSerp?.checked;

      app.greatDeals.hybrid.query = query;
      app.greatDeals.hybrid.minPrice = minPrice;
      app.greatDeals.hybrid.maxPrice = maxPrice;
      app.greatDeals.hybrid.minProfit = minProfit;
      app.greatDeals.hybrid.distance = distance;
      app.greatDeals.hybrid.city = city;
      app.greatDeals.hybrid.sourceEbay = sourceEbay;
      app.greatDeals.hybrid.sourceCraigslist = sourceCraigslist;
      app.greatDeals.hybrid.sourceFacebook = sourceFacebook;
      app.greatDeals.hybrid.sourceSerp = sourceSerp;
      app.greatDeals.hybrid.lastAutoRunAt = Date.now();

      if (!query) {
        gdHybridSetStatus(auto ? "Auto scan waiting: set query first" : "Enter search query first");
        gdRenderAutoScanMeta();
        saveSettingsLocal();
        return;
      }

      gdHybridSetStatus("Scanning Deal Spotter...");
      const response = await fetch("/api/great-deals/scan", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          keyword: query,
          category: String(el.gdCategory?.value || "Other"),
          minPrice,
          maxPrice,
          minProfit,
          distance,
          ...
          pickupCost: 0,
          riskPenalty: 0,
          autoAlert: app.greatDeals.hybrid.autoSendOnScan !== false,
          sources: {
            facebook: sourceFacebook,
            craigslist: sourceCraigslist,
            ebay: sourceEbay,
            serp: sourceSerp
          }
        })
      ...
      if (!response.ok || data?.ok === false) {
        ...
        gdHybridSetStatus(`Scan failed: ${err}`);
        gdRenderAutoScanMeta();
        saveSettingsLocal();
        return;
      }
      const rows = Array.isArray(data?.candidates) ? data.candidates : [];
      app.greatDeals.hybrid.candidates = rows.slice(0, 120).map((x) => ({
        id: String(x.id || `cand_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`),
        source: String(x.source || "Unknown"),
        title: String(x.title || ""),
        askingPrice: gdNum(x.askingPrice),
        marketValue: gdNum(x.marketValue),
        profit: gdNum(x.profit),
        margin: gdNum(x.margin),
        score: gdNum(x.score),
        grade: String(x.grade || "Skip"),
        condition: String(x.condition || "good"),
        link: String(x.link || ""),
        imageUrl: String(x.imageUrl || ""),
        notes: String(x.notes || ""),
        suggestedOffer: gdNum(x.suggestedOffer),
        sellerTemplate: String(x.sellerTemplate || ""),
        comps: Array.isArray(x.comps) ? x.comps : [],
      }));
      const warnings = Array.isArray(data?.warnings) ? data.warnings : [];
      const alertsCount = gdNum(data?.counts?.alerts);
      const sourceCounts = {};
      app.greatDeals.hybrid.candidates.forEach((row) => {
      ...
      const sourceSummary = Object.entries(sourceCounts)
        ...
        .join(", ");
      gdHybridSetStatus(`Found ${app.greatDeals.hybrid.candidates.length} candidates${sourceSummary ? ` • sources: ${sourceSummary}` : ""}${alertsCount > 0 ? ` • sent ${alertsCount} alert(s)` : ""}${warnings.length ? ` • warnings: ${warnings.slice(0, 2).join(" | ")}` : ""}`);
      if (app.greatDeals.hybrid.autoSendOnScan && !alertsCount && app.greatDeals.hybrid.candidates.length > 0) {
        try {
          const result = await gdSendCandidatesToTelegram(app.greatDeals.hybrid.candidates, auto ? 3 : 8);
          if (result.sent > 0) {
            gdSetStatus(`Scan complete. Sent ${result.sent} A/A+ candidate(s) to Telegram.`);
          } else {
            gdSetStatus("Scan complete. No new A/A+ candidates to send.");
          }
        } catch (error) {
          gdSetStatus(`Scan complete. Telegram send failed: ${error?.message || "Unknown error"}`);
        }
      }
      gdRenderCandidates();
      gdRenderAutoScanMeta();
    ...
    function gdImportTopCandidate() {
      const first = (app.greatDeals.hybrid.candidates || [])[0];
      if (!first) {
        gdSetStatus("No candidates to import.");
        return;
      }
      gdImportCandidate(first.id);
    }

    function gdOpenSearchLaunchers() {
      const query = encodeURIComponent(String(el.gdScanQuery?.value || app.greatDeals.hybrid.query || "").trim());
      const city = String(el.gdScanCity?.value || app.greatDeals.hybrid.city || "cincinnati").trim().toLowerCase().replace(/[^a-z0-9\-]/g, "") || "cincinnati";
      const useFacebook = !!el.gdSourceFacebook?.checked;
      const useSerp = !!el.gdSourceSerp?.checked;
      const ebay = `https://www.ebay.com/sch/i.html?_nkw=${query}`;
      const facebook = `https://www.facebook.com/marketplace/search/?query=${query}`;
      const craigslist = `https://${city}.craigslist.org/search/sss?query=${query}`;
      const googleShopping = `https://www.google.com/search?tbm=shop&q=${query}`;
      const googleSearch = `https://www.google.com/search?q=${query}+marketplace+price`;
      const offerup = `https://offerup.com/search/?q=${query}`;
      window.open(ebay, "_blank", "noopener");
      window.open(craigslist, "_blank", "noopener");
      window.open(offerup, "_blank", "noopener");
      if (useFacebook) window.open(facebook, "_blank", "noopener");
      if (useSerp) {
        window.open(googleShopping, "_blank", "noopener");
        window.open(googleSearch, "_blank", "noopener");
      }
      gdHybridSetStatus("Opened manual lanes: eBay, Craigslist, OfferUp, plus optional Facebook/Google comps.");
    }

    function gdCaptureFormState() {
      const input = gdBuildInputFromForm();
      app.greatDeals.form = {
        title: input.title,
        category: input.category,
        buyPrice: input.buyPrice ? String(input.buyPrice) : "",
        soldPrice: input.soldPrice ? String(input.soldPrice) : "",
        shippingCost: String(input.shippingCost),
        feePct: String(input.feePct),
        condition: input.condition,
        demand: String(input.demand || 3),
        risk: String(input.risk || 3),
        ease: String(input.ease || 3),
        link: input.link,
        notes: input.notes
      };
      return input;
    ...
    function gdCalculateAndRender() {
      const input = gdCaptureFormState();
      const calc = gdCompute(input);
      app.greatDeals.lastCalc = calc;
      gdRenderKpis(calc);
      gdSetStatus(`Calculated ${calc.grade} • Profit ${formatUsd(calc.netProfit)} • ROI ${formatPctSigned(calc.roi)}`);
      saveSettingsLocal();
      return { input, calc };
    }

    async function gdSendCurrentAlert(force = false) {
      const result = gdCalculateAndRender();
      const { input, calc } = result;
      if (!force && !["A+", "A"].includes(String(calc.grade || "").toUpperCase())) {
        gdSetStatus("Alert skipped. Auto-alert sends only A and A+ deals.");
        return;
      }
      const msg = gdTelegramMessage(input, calc);
      await gdSendTelegram(msg);
      gdSetStatus(`Telegram alert sent for ${input.title || "deal"} (${calc.grade}).`);
    }

    async function gdSaveDeal() {
      const { input, calc } = gdCalculateAndRender();
      if (!input.title) {
        gdSetStatus("Item title is required.");
        return;
      }
      if (input.buyPrice <= 0 || input.soldPrice <= 0) {
        gdSetStatus("Buy price and estimated sold price must be greater than zero.");
        return;
      }
      const id = app.greatDeals.editingId || `deal_${Date.now()}`;
      const payload = {
        id,
        createdAt: Date.now(),
        title: input.title,
      ...
      const idx = app.greatDeals.deals.findIndex((d) => String(d.id) === String(id));
      if (idx >= 0) app.greatDeals.deals[idx] = gdNormalizeDealRow({ ...app.greatDeals.deals[idx], ...payload });
      else app.greatDeals.deals.unshift(gdNormalizeDealRow(payload));
      app.greatDeals.editingId = "";
      try {
        const res = await fetch("/api/great-deals/deals", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            ...
            notes: payload.notes,
            askingPrice: payload.buyPrice,
            marketValue: payload.soldPrice,
        ...
        if (res.ok && data?.ok && data?.deal) {
          const d = gdNormalizeDealRow(data.deal);
          const localIdx = app.greatDeals.deals.findIndex((x) => String(x.id) === String(d.id));
          if (localIdx >= 0) app.greatDeals.deals[localIdx] = d;
        }
      } catch {
      ...
      saveSettingsLocal();
      gdRenderDealsTable();
      gdSetStatus(`Deal saved (${calc.grade}).`);
      if (el.gdAutoAlert?.checked && ["A+", "A"].includes(String(calc.grade || "").toUpperCase())) {
        try {
          await gdSendCurrentAlert(false);
        } catch (error) {
          gdSetStatus(`Saved. Telegram failed: ${error?.message || "Unknown error"}`);
        }
      }
    }

    function gdLoadDealToForm(id) {
      const deal = app.greatDeals.deals.find((d) => String(d.id) === String(id));
      if (!deal) return;
      app.greatDeals.editingId = String(id);
      app.greatDeals.form = {
        title: String(deal.title || ""),
        category: String(deal.category || "Other"),
        buyPrice: String(deal.buyPrice || ""),
        soldPrice: String(deal.soldPrice || ""),
        shippingCost: String(deal.shippingCost || "0"),
        feePct: String(deal.feePct || "13"),
        condition: String(deal.condition || "good"),
        demand: String(deal.demand || "3"),
        risk: String(deal.risk || "3"),
        ease: String(deal.ease || "3"),
        link: String(deal.link || ""),
        notes: String(deal.notes || "")
      };
      app.greatDeals.lastCalc = gdCompute({
        ...gdBuildInputFromForm(),
        ...deal
      });
      gdSyncFormFromState();
      gdRenderKpis(app.greatDeals.lastCalc);
      gdSetStatus(`Loaded deal for edit: ${deal.title}`);
      saveSettingsLocal();
    }

    function gdDeleteDeal(id) {
      app.greatDeals.deals = app.greatDeals.deals.filter((d) => String(d.id) !== String(id));
      if (String(app.greatDeals.editingId) === String(id)) app.greatDeals.editingId = "";
      fetch(`/api/great-deals/deals/${encodeURIComponent(String(id))}`, { method: "DELETE" }).catch(() => {});
      saveSettingsLocal();
      gdRenderDealsTable();
      gdSetStatus("Deal deleted.");
    }

    async function gdUpdateDealStatus(id, status) {
      const safeStatus = String(status || "").trim().toLowerCase();
      if (!safeStatus) return;
      const row = app.greatDeals.deals.find((d) => String(d.id) === String(id));
      if (row) row.status = safeStatus;
      gdRenderDealsTable();
      saveSettingsLocal();
      try {
        await fetch(`/api/great-deals/deals/${encodeURIComponent(String(id))}/status`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      gdSetStatus(`Marked as ${safeStatus}.`);
    }

    async function gdAlertDeal(id) {
      const deal = app.greatDeals.deals.find((d) => String(d.id) === String(id));
      if (!deal) return;
      const calc = gdCompute({
        title: deal.title,
        category: deal.category,
        buyPrice: deal.buyPrice,
        soldPrice: deal.soldPrice,
      ...
      const msg = gdTelegramMessage(deal, calc);
      await gdSendTelegram(msg);
      gdSetStatus(`Telegram alert sent for saved deal: ${deal.title}`);
    }

    function parseAdolWatchlist(value) {
      return String(value || "")
        .split(/[,\s;\n\r]+/)
    ...
    function getAdolTableState() {
      if (!app.adol.table || typeof app.adol.table !== "object") {
        app.adol.table = {
          sortKey: "finalAiScore",
          sortDir: "desc",
          trend: "ALL",
      ...
      return app.adol.table;
    }

    function adolNum(value) {
      const n = Number(value);
      return Number.isFinite(n) ? n : NaN;
    }

    function adolSortValue(row, key) {
      if (!row || !key) return "";
      const numberKeys = new Set(["price", "changePct", "gapPct", "rvol", "bullScore", "bearScore", "socialSentimentScore", "newsSentimentScore", "finalAiScore", "entry", "stop", "t1", "t2", "t3", "topTimingScore", "bottomTimingScore"]);
      if (numberKeys.has(key)) return adolNum(row[key]);
      return String(row[key] || "").toLowerCase();
    }

    function applyAdolTableFiltersAndSort(rows) {
      const state = getAdolTableState();
      let out = Array.isArray(rows) ? [...rows] : [];

      if (state.trend && state.trend !== "ALL") {
        out = out.filter((r) => String(r?.trend || "").toLowerCase() === String(state.trend).toLowerCase());
      }
      if (state.action && state.action !== "ALL") {
        out = out.filter((r) => String(r?.action || "").toLowerCase() === String(state.action).toLowerCase());
      }

      const mins = [
        ["minBull", "bullScore"],
        ["minBear", "bearScore"],
        ["minSocial", "socialSentimentScore"],
        ["minNews", "newsSentimentScore"],
        ["minFinalAi", "finalAiScore"]
      ];
      mins.forEach(([stateKey, rowKey]) => {
        const raw = String(state[stateKey] ?? "").trim();
        if (!raw) return;
        const n = Number(raw);
        if (!Number.isFinite(n)) return;
        out = out.filter((r) => Number(r?.[rowKey] ?? -9999) >= n);
      });

      const sortKey = String(state.sortKey || "finalAiScore");
      const dir = String(state.sortDir || "desc") === "asc" ? 1 : -1;
      out.sort((a, b) => {
        const va = adolSortValue(a, sortKey);
        const vb = adolSortValue(b, sortKey);
        const na = Number(va);
        const nb = Number(vb);
    ...
    function adolHeaderCell(label, key) {
      const state = getAdolTableState();
      const mark = state.sortKey === key ? (state.sortDir === "asc" ? " ?" : " ?") : "";
      return `<th class="adol-sortable" onclick="window.adolSetTableSort && window.adolSetTableSort('${key}')">${escapeHtml(label + mark)}</th>`;
    }

    function adolToneClassByAction(actionText, row = {}) {
      const action = String(actionText || "").toLowerCase();
      const trend = String(row?.trend || "").toLowerCase();
      const bull = adolNum(row?.bullScore);
      const bear = adolNum(row?.bearScore);
      if (action.includes("caution") || action.includes("no trade")) return "yellow";
      if (action.includes("wait") || action.includes("watch")) return "blue";
      if (action.includes("short") || action.includes("avoid") || action.includes("bear")) return "red";
      if (action.includes("long") || action.includes("buy") || action.includes("bull")) return "green";
      if (trend.includes("down")) return "red";
      if (trend.includes("up")) return "green";
      if (bull > bear) return "green";
      if (bear > bull) return "red";
      return "blue";
    }

    function adolToneClassForTrend(trendText) {
      const trend = String(trendText || "").toLowerCase();
      if (trend.includes("down")) return "red";
      if (trend.includes("up")) return "green";
      if (trend.includes("range")) return "yellow";
      return "blue";
    }

    function adolToneClassForVwap(vwapText) {
      const text = String(vwapText || "").toLowerCase();
      if (text.includes("buyer")) return "green";
      if (text.includes("seller")) return "red";
      return "blue";
    }

    function adolToneClassForTrap(trapText) {
      const text = String(trapText || "").toLowerCase();
      if (!text || text === "none") return "blue";
      if (text.includes("bull trap") || text.includes("bear trap") || text.includes("hype trap")) return "red";
      if (text.includes("trap")) return "yellow";
      if (text.includes("reclaim")) return "green";
      return "yellow";
    }

    function adolToneClassForTiming(signalText) {
      const text = String(signalText || "").toLowerCase();
      if (text.includes("potential top")) return "red";
      if (text.includes("potential bottom")) return "green";
      if (text.includes("volatile reversal")) return "yellow";
      return "blue";
    }

    function adolToneChip(text, tone) {
      const safeTone = ["green", "red", "yellow", "blue"].includes(tone) ? tone : "blue";
      return `<span class="adol-tone ${safeTone}">${escapeHtml(String(text || "-"))}</span>`;
    }

    // Heat Risk Engine (frontend, per selected stock click)
    const ADOL_HEAT_SIGNALS = {
      HEALTHY_STRENGTH: "HEALTHY_STRENGTH",
      OVEREXTENDED_DO_NOT_CHASE: "OVEREXTENDED_DO_NOT_CHASE",
      CLIMACTIC_DANGER: "CLIMACTIC_DANGER",
      WEAK_AVOID: "WEAK_AVOID",
      NEUTRAL_WAIT: "NEUTRAL_WAIT"
    };

    const ADOL_HEAT_COLORS = {
      HEALTHY_STRENGTH: "green",
      OVEREXTENDED_DO_NOT_CHASE: "orange",
      CLIMACTIC_DANGER: "red",
      WEAK_AVOID: "gray",
      NEUTRAL_WAIT: "yellow"
    };

    const ADOL_HEAT_ACTIONS = {
      HEALTHY_STRENGTH: "Hold / Buy pullback",
      OVEREXTENDED_DO_NOT_CHASE: "Wait for pullback",
      CLIMACTIC_DANGER: "Trim / Do not chase",
      WEAK_AVOID: "Avoid / Exit",
      NEUTRAL_WAIT: "Wait"
    };

    const ADOL_HEAT_RISK = {
      HEALTHY_STRENGTH: "Controlled",
      OVEREXTENDED_DO_NOT_CHASE: "Medium-High",
      CLIMACTIC_DANGER: "High",
      WEAK_AVOID: "High",
      NEUTRAL_WAIT: "Mixed"
    };

    function adolClamp(value, min, max) {
      const v = Number(value);
      if (!Number.isFinite(v)) return Number(min);
      return Math.max(Number(min), Math.min(Number(max), v));
    }

    function adolSafeNumber(value, fallback = 0) {
      const num = Number(value);
      return Number.isFinite(num) ? num : fallback;
    }

    function adolYesNo(value) {
      return value === true || value === "true" || value === "yes" || value === "YES";
    }

    function adolHeatToneClass(signal) {
      const color = ADOL_HEAT_COLORS[signal] || "yellow";
      if (color === "green") return "good";
      if (color === "red") return "risk";
      if (color === "yellow" || color === "orange") return "warn";
      if (color === "gray") return "risk";
      return "warn";
    }

    function adolGetHeatScore(data, signal) {
      const rsi = adolSafeNumber(data.rsi);
      const relativeVolume = adolSafeNumber(data.relativeVolume);
      const percentAbove21Ema = adolSafeNumber(data.percentAbove21Ema);
      const percentAboveVwap = adolSafeNumber(data.percentAboveVwap);

      const priceAbove9Ema = adolYesNo(data.priceAbove9Ema);
      const priceAbove21Ema = adolYesNo(data.priceAbove21Ema);
      const priceAbove50Sma = adolYesNo(data.priceAbove50Sma);
      const trendStackBullish = adolYesNo(data.trendStackBullish);

      let score = 50;
      if (priceAbove9Ema) score += 8;
      if (priceAbove21Ema) score += 8;
      if (priceAbove50Sma) score += 8;
      if (trendStackBullish) score += 12;

      if (rsi >= 55 && rsi <= 72) score += 12;
      if (rsi > 72 && rsi < 75) score += 3;
      if (rsi >= 75) score -= 10;
      if (rsi < 50) score -= 15;

      if (relativeVolume >= 1.1 && relativeVolume < 2.0) score += 10;
      if (relativeVolume >= 2.0) score -= 5;

      if (percentAbove21Ema >= 0 && percentAbove21Ema < 8) score += 12;
      if (percentAbove21Ema >= 8 && percentAbove21Ema < 10) score -= 8;
      if (percentAbove21Ema >= 10) score -= 15;
      if (percentAboveVwap > 5) score -= 5;

      if (signal === ADOL_HEAT_SIGNALS.HEALTHY_STRENGTH) score = Math.max(score, 75);
      if (signal === ADOL_HEAT_SIGNALS.OVEREXTENDED_DO_NOT_CHASE) score = Math.min(score, 68);
      if (signal === ADOL_HEAT_SIGNALS.CLIMACTIC_DANGER) score = Math.min(score, 55);
      if (signal === ADOL_HEAT_SIGNALS.WEAK_AVOID) score = Math.min(score, 45);
      if (signal === ADOL_HEAT_SIGNALS.NEUTRAL_WAIT) score = Math.min(score, 60);

      return adolClamp(Math.round(score), 0, 100);
    }

    function adolBuildHeatReason(data, signal) {
      const rsi = adolSafeNumber(data.rsi);
      const relativeVolume = adolSafeNumber(data.relativeVolume);
      const percentAbove21Ema = adolSafeNumber(data.percentAbove21Ema);
      const trendStackBullish = adolYesNo(data.trendStackBullish);

      if (signal === ADOL_HEAT_SIGNALS.CLIMACTIC_DANGER) {
        return `Climactic danger: RSI ${formatNum(rsi, 1)}, relative volume ${formatNum(relativeVolume, 2)}, and price ${formatNum(percentAbove21Ema, 2)}% above 21 EMA.`;
      }
      if (signal === ADOL_HEAT_SIGNALS.OVEREXTENDED_DO_NOT_CHASE) {
        return `Overextended: trend still bullish, but RSI ${formatNum(rsi, 1)} and extension ${formatNum(percentAbove21Ema, 2)}% above 21 EMA make entry risk poor.`;
      }
      if (signal === ADOL_HEAT_SIGNALS.HEALTHY_STRENGTH) {
        return `Healthy strength: trend is controlled, RSI ${formatNum(rsi, 1)}, RVOL ${formatNum(relativeVolume, 2)} and extension remains acceptable.`;
      }
      if (signal === ADOL_HEAT_SIGNALS.WEAK_AVOID) {
        return "Weak setup: trend stack is not clean, and momentum is not confirming upside.";
      }
      return `Neutral: mixed signals (Trend bullish: ${trendStackBullish ? "yes" : "no"}, RSI ${formatNum(rsi, 1)}, RVOL ${formatNum(relativeVolume, 2)}).`;
    }

    function adolBuildHeatEntryPlan(data, signal) {
      const price = adolSafeNumber(data.price);
      const support = adolSafeNumber(data.support);
      const resistance = adolSafeNumber(data.resistance);
      const percentAbove21Ema = adolSafeNumber(data.percentAbove21Ema);

      if (signal === ADOL_HEAT_SIGNALS.HEALTHY_STRENGTH) {
        return `Preferred entry near support ${support > 0 ? formatUsd(support) : "zone"} or clean continuation above ${resistance > 0 ? formatUsd(resistance) : "resistance"}.`;
      }
      if (signal === ADOL_HEAT_SIGNALS.OVEREXTENDED_DO_NOT_CHASE) {
        return `Do not chase near ${formatUsd(price)}. Wait for pullback into 9EMA/21EMA (currently ${formatNum(percentAbove21Ema, 2)}% above 21EMA).`;
      }
      if (signal === ADOL_HEAT_SIGNALS.CLIMACTIC_DANGER) {
        return "No fresh entry. Let move cool and wait for consolidation or reclaim setup.";
      }
      if (signal === ADOL_HEAT_SIGNALS.WEAK_AVOID) {
        return "Avoid long entry until price reclaims 21EMA and trend stack improves.";
      }
      return "Wait for cleaner confirmation: VWAP reclaim + volume or valid resistance break.";
    }

    function adolBuildHeatStopLoss(data, signal) {
      const support = adolSafeNumber(data.support);
      const price = adolSafeNumber(data.price);
      if (support > 0) return support;
      if (signal === ADOL_HEAT_SIGNALS.HEALTHY_STRENGTH) return Number((price * 0.97).toFixed(2));
      if (signal === ADOL_HEAT_SIGNALS.OVEREXTENDED_DO_NOT_CHASE) return Number((price * 0.95).toFixed(2));
      if (signal === ADOL_HEAT_SIGNALS.CLIMACTIC_DANGER) return Number((price * 0.96).toFixed(2));
      return Number((price * 0.95).toFixed(2));
    }

    function adolBuildHeatTargets(data) {
      const price = adolSafeNumber(data.price);
      const resistance = adolSafeNumber(data.resistance);
      const target1 = resistance > price ? resistance : price * 1.03;
      const target2 = price * 1.06;
      const target3 = price * 1.10;
      return [
        Number(target1.toFixed(2)),
        Number(target2.toFixed(2)),
        Number(target3.toFixed(2))
      ];
    }

    function adolBuildHeatWarnings(data, signal) {
      const warnings = [];
      const rsi = adolSafeNumber(data.rsi);
      const relativeVolume = adolSafeNumber(data.relativeVolume);
      const percentAbove21Ema = adolSafeNumber(data.percentAbove21Ema);
      const upperWickSize = String(data.upperWickSize || "").toLowerCase();
      const candleClosePosition = String(data.candleClosePosition || "").toLowerCase();

      if (rsi >= 75) warnings.push("RSI is very hot.");
      if (relativeVolume >= 2.0) warnings.push("Relative volume is extreme.");
      if (percentAbove21Ema >= 10) warnings.push("Price is far above 21EMA.");
      if (upperWickSize === "large") warnings.push("Large upper wick may signal sellers.");
      if (candleClosePosition === "low" || candleClosePosition === "weak") warnings.push("Weak candle close may signal exhaustion.");
      if (signal === ADOL_HEAT_SIGNALS.CLIMACTIC_DANGER) warnings.push("Do not chase. Pullback risk is high.");
      if (signal === ADOL_HEAT_SIGNALS.OVEREXTENDED_DO_NOT_CHASE) warnings.push("Strong stock, but entry risk is poor.");
      if (!warnings.length) warnings.push("No major warning signs detected.");
      return warnings;
    }

    function adolClassifyHeatRiskFromRow(row = {}) {
      const price = adolSafeNumber(row.price);
      const ema9 = adolSafeNumber(row.ema9);
      const ema21 = adolSafeNumber(row.ema21);
      const ema200 = adolSafeNumber(row.ema200);
      const vwap = adolSafeNumber(row.vwap);
      const rsi = adolSafeNumber(row.rsi, 50);
      const relativeVolume = adolSafeNumber(row.rvol, 1);
      const percentAbove21Ema = ema21 > 0 ? ((price - ema21) / ema21) * 100 : 0;
      const percentAboveVwap = vwap > 0 ? ((price - vwap) / vwap) * 100 : 0;
      const trendStackBullish = ema9 > ema21 && ema21 > ema200;
      const priceAbove21Ema = price > ema21;
      const priceAbove9Ema = price > ema9;
      const priceAbove50Sma = priceAbove21Ema;

      const upperWickSize = String(row.upperWickSize || "").toLowerCase();
      const candleClosePosition = String(row.candleClosePosition || "").toLowerCase();
      const bigGreenCandlesCount = adolSafeNumber(row.bigGreenCandlesCount);
      const hasExhaustion =
        upperWickSize === "large" ||
        candleClosePosition === "weak" ||
        candleClosePosition === "low" ||
        bigGreenCandlesCount >= 4;

      let signal = ADOL_HEAT_SIGNALS.NEUTRAL_WAIT;
      if (rsi >= 75 && relativeVolume >= 2.0 && percentAbove21Ema >= 10) {
        signal = ADOL_HEAT_SIGNALS.CLIMACTIC_DANGER;
      } else if (trendStackBullish && rsi >= 75 && percentAbove21Ema >= 10 && hasExhaustion) {
        signal = ADOL_HEAT_SIGNALS.CLIMACTIC_DANGER;
      } else if (trendStackBullish && rsi >= 70 && percentAbove21Ema >= 8) {
        signal = ADOL_HEAT_SIGNALS.OVEREXTENDED_DO_NOT_CHASE;
      } else if (trendStackBullish && rsi >= 55 && rsi <= 72 && relativeVolume >= 1.1 && percentAbove21Ema < 8) {
        signal = ADOL_HEAT_SIGNALS.HEALTHY_STRENGTH;
      } else if (!priceAbove21Ema || (!trendStackBullish && rsi < 50)) {
        signal = ADOL_HEAT_SIGNALS.WEAK_AVOID;
      }

      const score = adolGetHeatScore({
        rsi,
        relativeVolume,
        percentAbove21Ema,
        percentAboveVwap,
      ...
      const support = adolSafeNumber(row.stopHuntZones?.priorDayLow) || adolSafeNumber(row.stop);
      const resistance = adolSafeNumber(row.stopHuntZones?.priorDayHigh) || adolSafeNumber(row.t1);
      const stopLoss = adolBuildHeatStopLoss({ support, price }, signal);
      const targets = adolBuildHeatTargets({ price, resistance });

      return {
        signal,
        color: ADOL_HEAT_COLORS[signal] || "yellow",
        score,
        action: ADOL_HEAT_ACTIONS[signal],
        risk: ADOL_HEAT_RISK[signal],
        buyNow: signal === ADOL_HEAT_SIGNALS.HEALTHY_STRENGTH && rsi < 70 && percentAbove21Ema < 6,
        reason: adolBuildHeatReason({ rsi, relativeVolume, percentAbove21Ema, trendStackBullish }, signal),
        entryPlan: adolBuildHeatEntryPlan({ price, support, resistance, percentAbove21Ema }, signal),
        stopLoss,
        targets,
        warnings: adolBuildHeatWarnings({
          rsi,
          relativeVolume,
          percentAbove21Ema,
          upperWickSize,
    ...
    function adolTopCandidate(rows) {
      const list = Array.isArray(rows) ? rows : [];
      if (!list.length) return null;
      const ranked = [...list].sort((a, b) => Number(b.finalAiScore || 0) - Number(a.finalAiScore || 0));
      return ranked[0] || null;
    }

    function adolRiskProfile() {
      const mode = String(app?.adol?.riskMode || "balanced").toLowerCase();
      if (mode === "aggressive") return { mode: "aggressive", label: "Aggressive", stopMult: 0.8, t1: 1.2, t2: 2.2, t3: 3.4 };
      if (mode === "conservative") return { mode: "conservative", label: "Conservative", stopMult: 1.25, t1: 0.9, t2: 1.6, t3: 2.3 };
      return { mode: "balanced", label: "Balanced", stopMult: 1.0, t1: 1.0, t2: 2.0, t3: 3.0 };
    }

    function adolTradeSide(row) {
      const action = String(row?.action || row?.institutionalAction || "").toLowerCase();
      const trend = String(row?.trend || "").toLowerCase();
      const bull = Number(row?.bullScore || 0);
      const bear = Number(row?.bearScore || 0);
      if (action.includes("short") || action.includes("sell") || action.includes("bear")) return "short";
      if (action.includes("long") || action.includes("buy") || action.includes("bull")) return "long";
      if (trend.includes("down")) return "short";
      if (trend.includes("up")) return "long";
      return bear > bull ? "short" : "long";
    }

    function adolApplySideFilter(rows) {
      const list = Array.isArray(rows) ? rows : [];
      const mode = String(app?.adol?.sideMode || "both").toLowerCase();
      if (mode === "both") return list;
      return list.filter((r) => adolTradeSide(r) === mode);
    }

    function adolComputePlan(row) {
      if (!row || typeof row !== "object") return null;
      const profile = adolRiskProfile();
      const entry = Number(row.entry);
      if (!Number.isFinite(entry)) return null;
      const rawStop = Number(row.stop);
      const rawT1 = Number(row.t1);
      const baseRisk = (() => {
        if (Number.isFinite(rawStop) && Math.abs(entry - rawStop) > 0) return Math.abs(entry - rawStop);
        if (Number.isFinite(rawT1) && Math.abs(rawT1 - entry) > 0) return Math.abs(rawT1 - entry);
        return Math.max(entry * 0.012, 0.01);
      })();
      const side = adolTradeSide(row);
      const stopDist = baseRisk * profile.stopMult;
      const stop = side === "short" ? entry + stopDist : entry - stopDist;
      const t1 = side === "short" ? entry - stopDist * profile.t1 : entry + stopDist * profile.t1;
      const t2 = side === "short" ? entry - stopDist * profile.t2 : entry + stopDist * profile.t2;
      const t3 = side === "short" ? entry - stopDist * profile.t3 : entry + stopDist * profile.t3;
      return { side, profile, entry, stop, t1, t2, t3, risk: stopDist };
    }

    function adolSelectedCandidate(rows) {
      const list = adolApplySideFilter(Array.isArray(rows) ? rows : []);
      if (!list.length) return null;
      const selected = String(app?.adol?.selectedTicker || "").trim().toUpperCase();
      if (selected) {
        const found = list.find((r) => String(r?.ticker || "").trim().toUpperCase() === selected);
        if (found) return found;
      }
      return adolTopCandidate(list);
    }

    function adolZoneText(low, high) {
      const nLow = Number(low);
      const nHigh = Number(high);
      if (!Number.isFinite(nLow) || !Number.isFinite(nHigh)) return "-";
      const lo = Math.min(nLow, nHigh);
      const hi = Math.max(nLow, nHigh);
      return `${formatUsd(lo)} - ${formatUsd(hi)}`;
    }

    function adolProjectionLabel(row) {
      const finalAi = Number(row?.finalAiScore || 0);
      const bull = Number(row?.bullScore || 0);
      const bear = Number(row?.bearScore || 0);
      const trend = String(row?.trend || "").toLowerCase();
      if (finalAi >= 85 && bull > bear) return "High Conviction Bullish";
      if (finalAi >= 75 && bear > bull) return "High Conviction Bearish";
      if (trend.includes("up")) return "Moderate Bullish Drift";
      if (trend.includes("down")) return "Moderate Bearish Drift";
      return "Range / Wait For Confirmation";
    }

    function adolTradeIdeaLines(row) {
      const lines = [];
      const entry = Number(row?.entry);
      const stop = Number(row?.stop);
      const t1 = Number(row?.t1);
      const t2 = Number(row?.t2);
      const t3 = Number(row?.t3);
      const finalAi = Number(row?.finalAiScore || 0);
      const action = String(row?.action || "").toLowerCase();
      if (Number.isFinite(entry) && Number.isFinite(stop)) {
        const risk = Math.abs(entry - stop);
        if (risk > 0) {
          const rr1 = Number.isFinite(t1) ? (Math.abs(t1 - entry) / risk) : NaN;
          const rr2 = Number.isFinite(t2) ? (Math.abs(t2 - entry) / risk) : NaN;
          lines.push(`Risk per share: ${formatUsd(risk)}${Number.isFinite(rr1) ? ` • R/R to T1: ${rr1.toFixed(2)}R` : ""}${Number.isFinite(rr2) ? ` • R/R to T2: ${rr2.toFixed(2)}R` : ""}`);
        }
      }
      if (finalAi >= 85) lines.push("Execution idea: scale in 50/30/20 around entry zone and trail stop after T1.");
      else if (finalAi >= 75) lines.push("Execution idea: partial size, wait for confirmation candle before full entry.");
      else if (action.includes("watch")) lines.push("Execution idea: keep on watchlist, trade only after breakout/reclaim confirmation.");
      else lines.push("Execution idea: no trade now, wait for cleaner setup alignment.");
      return lines;
    }

    function adolDecisionPanelHtml(rows) {
      const best = adolSelectedCandidate(rows);
      if (!best) return "";
      const plan = adolComputePlan(best);
      if (!plan) return "";
      const { entry, stop, t1, t2, t3, risk, side, profile } = plan;
      const buyArea = Number.isFinite(entry) && Number.isFinite(stop)
        ? adolZoneText(entry - Math.abs(entry - stop) * 0.35, entry + Math.abs(entry - stop) * 0.25)
        : formatUsd(entry);
      const sellArea = Number.isFinite(t2) && Number.isFinite(t3) ? adolZoneText(t2, t3) : formatUsd(t2);
      const holdArea = Number.isFinite(entry) && Number.isFinite(t1) ? adolZoneText(entry, t1) : "-";
      const projection = adolProjectionLabel(best);
      const planLines = adolTradeIdeaLines(best);
      return `
        <div class="tile" style="margin-bottom:10px;">
          <div class="row" style="justify-content:space-between;gap:8px;align-items:flex-start;">
            <div class="k">Pro Trade Plan • ${escapeHtml(String(best.ticker || "-"))}</div>
            <span class="badge">${escapeHtml(String(best.action || "Watch"))} • ${escapeHtml(profile.label)} • ${side.toUpperCase()} • Final AI ${formatNum(best.finalAiScore, 0)}</span>
          </div>
          <div class="adol-decision-grid">
            <div class="adol-decision-card buy">
              <div class="k">Buy Area</div>
              <div class="v">${escapeHtml(buyArea)}</div>
              <div class="s">Preferred entry zone near reclaim/support.</div>
            </div>
            <div class="adol-decision-card sell">
              <div class="k">Sell Area</div>
              <div class="v">${escapeHtml(sellArea)}</div>
              <div class="s">Take-profit zone around projected extension.</div>
            </div>
            <div class="adol-decision-card hold">
              <div class="k">Hold Zone</div>
              <div class="v">${escapeHtml(holdArea)}</div>
              <div class="s">Manage position while structure stays intact.</div>
            </div>
            <div class="adol-decision-card proj">
              <div class="k">Projection</div>
              <div class="v" style="font-size:19px;">${escapeHtml(projection)}</div>
              <div class="s">Targets: ${escapeHtml(formatUsd(t1))} / ${escapeHtml(formatUsd(t2))} / ${escapeHtml(formatUsd(t3))}</div>
            </div>
          </div>
          <ul class="adol-plan-list">
            <li>Mode: ${escapeHtml(profile.label)} • Direction: ${escapeHtml(side.toUpperCase())} • Risk per share: ${formatUsd(risk)}</li>
    ...
    function adolDeepDiveHtml(rows) {
      const best = adolSelectedCandidate(rows);
      if (!best) {
        return `
          <div id="adolDeepDivePanel" class="tile" style="margin-bottom:10px;">
            <div class="k">Deep Dive</div>
            <div style="font-size:16px;font-weight:700;margin-top:6px;">No candidate selected yet</div>
            <div style="color:#9ec2e3;margin-top:6px;">Run ADOL scan, then click any ticker row to load Heat Risk Engine, Heat Stop/Targets, and Heat Engine Notes.</div>
          </div>
        `;
      }
      const plan = adolComputePlan(best);
      if (!plan) {
        return `
          <div id="adolDeepDivePanel" class="tile" style="margin-bottom:10px;">
            <div class="k">Deep Dive • ${escapeHtml(String(best.ticker || "-"))}</div>
            <div style="font-size:16px;font-weight:700;margin-top:6px;">Plan unavailable</div>
            <div style="color:#9ec2e3;margin-top:6px;">This row is missing entry/stop levels. Try another ticker row.</div>
          </div>
        `;
      }

      const side = String(plan.side || "long").toUpperCase();
      const finalAi = Number(best?.finalAiScore || 0);
      const bull = Number(best?.bullScore || 0);
      const bear = Number(best?.bearScore || 0);
      const social = Number(best?.socialSentimentScore || 0);
      const news = Number(best?.newsSentimentScore || 0);
      const rvol = Number(best?.rvol || 0);
      const trend = String(best?.trend || "-");
      const vwapControl = String(best?.vwapControl || (bull >= bear ? "Buyers in Control" : "Sellers in Control"));
      const liquidityEvent = String(best?.liquidityEvent || "None");
      const trapSignal = String(best?.trapSignal || "None");
      const timingSignal = String(best?.timingSignal || "Trend Continuation");
      const timingBias = String(best?.timingBias || "Neutral");
      const topTimingScore = Number(best?.topTimingScore || 0);
      const bottomTimingScore = Number(best?.bottomTimingScore || 0);
      const timingNote = String(best?.timingNote || "No clear exhaustion signal.");
      const optionsBias = String(best?.optionsBias || "Options flow disabled");
      const projection = adolProjectionLabel(best);
      const heatModel = adolClassifyHeatRiskFromRow(best);
      const heatTone = adolHeatToneClass(heatModel?.signal);
      const heatTargets = Array.isArray(heatModel?.targets) ? heatModel.targets : [plan.t1, plan.t2, plan.t3];
      const heatWarnings = Array.isArray(heatModel?.warnings) ? heatModel.warnings : [];
      const deepSnap = (String(app?.deepDive?.ticker || "").toUpperCase() === String(best?.ticker || "").toUpperCase())
        ? (app.deepDive || {})
        : {};
      const deepLoading = !!deepSnap?.loading;
      const deepTech = deepSnap?.technical || {};
      const deepFund = deepSnap?.fundamentals || {};
      const deepUpdated = deepSnap?.updatedAt ? new Date(deepSnap.updatedAt).toLocaleTimeString() : "";
      const riskPct = plan.entry > 0 ? (Math.abs(plan.entry - plan.stop) / plan.entry) * 100 : 0;
      const rrToT2 = Math.abs(plan.t2 - plan.entry) / Math.max(Math.abs(plan.entry - plan.stop), 0.0001);
      const setupStrength = finalAi >= 85 ? "A+ setup quality" : finalAi >= 75 ? "Strong watch setup" : finalAi >= 65 ? "Watchlist quality" : "Low quality / wait";
      const invalidation = side === "LONG"
        ? `Close below ${formatUsd(plan.stop)} with weak reclaim`
        : `Close above ${formatUsd(plan.stop)} with failed breakdown`;
      const vwapLower = String(vwapControl || "").toLowerCase();
      const buyersControl = vwapLower.includes("buyer");
      const sellersControl = vwapLower.includes("seller");
      const bullTrigger = buyersControl
        ? `Hold above VWAP and break/hold over ${formatUsd(plan.t1)}`
        : `Reclaim VWAP then hold above ${formatUsd(plan.entry)}`;
      const bearTrigger = sellersControl
        ? `Hold below VWAP and reject under ${formatUsd(plan.entry)}`
        : `Lose VWAP then fail reclaim under ${formatUsd(plan.entry)}`;
      const bullConfirm = `Bull>${formatNum(bear, 0)}? now ${formatNum(bull, 0)}/${formatNum(bear, 0)} • RVOL ${formatNum(rvol, 2)} • trap ${escapeHtml(trapSignal)}`;
      const bearConfirm = `Bear>${formatNum(bull, 0)}? now ${formatNum(bear, 0)}/${formatNum(bull, 0)} • RVOL ${formatNum(rvol, 2)} • trap ${escapeHtml(trapSignal)}`;
      const bullInvalidation = `Bull setup invalid below ${formatUsd(plan.stop)} or failed reclaim behavior`;
      const bearInvalidation = `Bear setup invalid above ${formatUsd(plan.stop)} or failed breakdown behavior`;

      return `
        <div id="adolDeepDivePanel" class="tile" style="margin-bottom:10px;">
          <div class="row" style="justify-content:space-between;gap:8px;align-items:flex-start;">
            <div class="k">Deep Dive • ${escapeHtml(String(best.ticker || "-"))}</div>
            <span class="badge">${escapeHtml(String(best.action || "Watch"))} • ${escapeHtml(side)} • Final AI ${formatNum(finalAi, 0)}</span>
          </div>

          <div class="adol-deep-grid">
            <div class="adol-deep-card good">
              <div class="k">Bias + Structure</div>
              <div class="v">${escapeHtml(projection)}</div>
              <div class="s">Trend: ${escapeHtml(trend)} • VWAP: ${escapeHtml(vwapControl)} • Setup: ${escapeHtml(setupStrength)}</div>
            </div>
            <div class="adol-deep-card warn">
              <div class="k">Momentum + Sentiment</div>
              <div class="v">Bull ${formatNum(bull, 0)} / Bear ${formatNum(bear, 0)}</div>
              <div class="s">RVOL ${formatNum(rvol, 2)} • Social ${formatNum(social, 0)} • News ${formatNum(news, 0)} • Options: ${escapeHtml(optionsBias)}</div>
            </div>
            <div class="adol-deep-card risk">
              <div class="k">Risk + Invalidation</div>
              <div class="v">${formatUsd(plan.entry)} ? ${formatUsd(plan.stop)}</div>
              <div class="s">Risk width ${formatNum(riskPct, 2)}% • R/R to T2 ${formatNum(rrToT2, 2)}R • Invalidation: ${escapeHtml(invalidation)}</div>
            </div>
            <div class="adol-deep-card ${topTimingScore >= 70 ? "risk" : (bottomTimingScore >= 70 ? "good" : "warn")}">
              <div class="k">Top / Bottom Timing</div>
              <div class="v">${escapeHtml(timingSignal)}</div>
              <div class="s">Top ${formatNum(topTimingScore, 0)} • Bottom ${formatNum(bottomTimingScore, 0)} • ${escapeHtml(timingBias)} • ${escapeHtml(timingNote)}</div>
            </div>
            <div class="adol-deep-card ${heatTone}">
              <div class="k">Heat Risk Engine</div>
              <div class="v">${escapeHtml(String(heatModel?.signal || "NEUTRAL_WAIT").replaceAll("_", " "))}</div>
              <div class="s">Score ${formatNum(heatModel?.score, 0)} • Risk ${escapeHtml(heatModel?.risk || "Mixed")} • Action ${escapeHtml(heatModel?.action || "Wait")} • BuyNow ${heatModel?.buyNow ? "Yes" : "No"}</div>
            </div>
          </div>

          <div class="adol-report-grid">
            <div class="tile">
              <div class="k">Instant Technical + Fundamental (${escapeHtml(String(best.ticker || "-"))})${deepUpdated ? ` • ${escapeHtml(deepUpdated)}` : ""}${deepLoading ? " • Loading..." : ""}</div>
              <ul class="adol-checklist">
                ${deepLoading ? "<li><b>Loading:</b> Pulling fresh technical + fundamental snapshot for this ticker...</li>" : ""}
                <li><b>Trend:</b> ${escapeHtml(String(deepTech?.trend || trend))} • <b>Momentum:</b> ${escapeHtml(String(deepTech?.momentum || "Neutral"))}</li>
                <li><b>EMA 9 / 21 / 200:</b> ${Number.isFinite(Number(deepTech?.ema9)) ? Number(deepTech.ema9).toFixed(2) : "--"} / ${Number.isFinite(Number(deepTech?.ema21)) ? Number(deepTech.ema21).toFixed(2) : "--"} / ${Number.isFinite(Number(deepTech?.ema200)) ? Number(deepTech.ema200).toFixed(2) : "--"}</li>
                <li><b>VWAP:</b> ${Number.isFinite(Number(deepTech?.vwap)) ? Number(deepTech.vwap).toFixed(2) : "--"} • <b>RSI:</b> ${Number.isFinite(Number(deepTech?.rsi)) ? Number(deepTech.rsi).toFixed(2) : "--"} • <b>MACD:</b> ${Number.isFinite(Number(deepTech?.macdLine)) ? Number(deepTech.macdLine).toFixed(3) : "--"}</li>
                <li><b>Market Cap:</b> ${formatCompact(deepFund?.marketCap || 0)} • <b>P/E:</b> ${Number.isFinite(Number(deepFund?.pe)) ? Number(deepFund.pe).toFixed(2) : "--"} • <b>EPS:</b> ${Number.isFinite(Number(deepFund?.eps)) ? Number(deepFund.eps).toFixed(2) : "--"}</li>
                <li><b>Sector:</b> ${escapeHtml(String(deepFund?.sector || "N/A"))}${deepFund?.industry ? ` / ${escapeHtml(String(deepFund.industry))}` : ""} • <b>Earnings:</b> ${escapeHtml(formatDateShort(deepFund?.earningsDate))}</li>
              </ul>
            </div>
            <div class="tile">
              <div class="k">Execution Checklist</div>
              <ul class="adol-checklist">
                <li>${side === "LONG" ? "Wait for reclaim and hold above VWAP before full size." : "Wait for rejection and hold below VWAP before full size."}</li>
                <li>Confirm liquidity event: ${escapeHtml(liquidityEvent)}</li>
                <li>Trap signal monitor: ${escapeHtml(trapSignal)}</li>
                <li>Scale plan: starter at entry, add on confirmation, reduce into T1/T2.</li>
              </ul>
            </div>
            <div class="tile">
              <div class="k">Price Map</div>
              <ul class="adol-checklist">
                <li>Entry: <b>${formatUsd(plan.entry)}</b></li>
                <li>Stop: <b>${formatUsd(plan.stop)}</b></li>
                <li>T1: <b>${formatUsd(plan.t1)}</b> • T2: <b>${formatUsd(plan.t2)}</b> • T3: <b>${formatUsd(plan.t3)}</b></li>
                <li>Action bias: <b>${escapeHtml(String(best.action || "Watch"))}</b></li>
                <li>Heat Stop: <b>${formatUsd(heatModel?.stopLoss)}</b> • Heat T1/T2/T3: <b>${formatUsd(heatTargets[0])}</b> / <b>${formatUsd(heatTargets[1])}</b> / <b>${formatUsd(heatTargets[2])}</b></li>
              </ul>
            </div>
            <div class="tile">
              <div class="k">Bull Scenario</div>
              <ul class="adol-checklist">
                <li><b>Trigger:</b> ${escapeHtml(bullTrigger)}</li>
                <li><b>Confirmation:</b> ${escapeHtml(bullConfirm)}</li>
                <li><b>Invalidation:</b> ${escapeHtml(bullInvalidation)}</li>
                <li><b>Path:</b> ${formatUsd(plan.entry)} ? ${formatUsd(plan.t1)} ? ${formatUsd(plan.t2)} ? ${formatUsd(plan.t3)}</li>
              </ul>
            </div>
            <div class="tile">
              <div class="k">Bear Scenario</div>
              <ul class="adol-checklist">
                <li><b>Trigger:</b> ${escapeHtml(bearTrigger)}</li>
                <li><b>Confirmation:</b> ${escapeHtml(bearConfirm)}</li>
                <li><b>Invalidation:</b> ${escapeHtml(bearInvalidation)}</li>
                <li><b>Path:</b> ${formatUsd(plan.entry)} ? ${formatUsd(plan.t1)} ? ${formatUsd(plan.t2)} ? ${formatUsd(plan.t3)}</li>
              </ul>
            </div>
            <div class="tile">
              <div class="k">Heat Engine Notes</div>
              <ul class="adol-checklist">
                <li><b>Reason:</b> ${escapeHtml(heatModel?.reason || "Neutral mix.")}</li>
                <li><b>Entry Plan:</b> ${escapeHtml(heatModel?.entryPlan || "Wait for cleaner confirmation.")}</li>
                ${heatWarnings.map((w) => `<li>${escapeHtml(String(w || ""))}</li>`).join("")}
              </ul>
            </div>
          </div>
        </div>
    ...
    function adolNormalizeNewsItem(item) {
      if (!item) return null;
      if (typeof item === "string") {
        const title = item.trim();
        if (!title) return null;
        return { title, source: "Scan feed", tag: "NEWS", impact: "Medium", url: "" };
      }
      const title = String(item.title || item.headline || item.text || "").trim();
      if (!title) return null;
      return {
        title,
        source: String(item.source || item.publisher || item.feed || "Scan feed"),
        tag: String(item.tag || item.topic || "NEWS"),
        impact: String(item.impact || item.sentiment || "Medium"),
        url: String(item.url || item.link || "")
      };
    }

    function adolExtractBigNews(scan) {
      ...
      const pools = [
        ...
        scan?.newsItems,
        report?.bigNews,
        report?.topNewsDrivers,
        app?.newsItems
      ];
      const merged = pools
        .flatMap((x) => Array.isArray(x) ? x : [])
        .map(adolNormalizeNewsItem)
        .filter(Boolean);
      const dedup = [];
      const seen = new Set();
    ...
    function adolExtractAnalystCalls(scan) {
      const report = scan?.report || {};
      const direct = [
        ...(Array.isArray(scan?.upgradesDowngrades) ? scan.upgradesDowngrades : []),
        ...(Array.isArray(scan?.analystCalls) ? scan.analystCalls : []),
        ...(Array.isArray(report?.upgradesDowngrades) ? report.upgradesDowngrades : [])
      ];

      const normalizeCall = (item) => {
        if (!item) return null;
        if (typeof item === "string") {
          const line = item.trim();
          if (!line) return null;
          const up = /upgrade|raised|outperform|overweight|buy/i.test(line);
          const down = /downgrade|cut|underperform|underweight|sell/i.test(line);
          return {
            ticker: "",
            action: up && !down ? "Upgrade" : down && !up ? "Downgrade" : "Analyst update",
            detail: line,
            source: "News",
            tone: up && !down ? "good" : down && !up ? "bad" : "warn"
          };
        }
        const detail = String(item.detail || item.note || item.title || item.headline || "").trim();
        const actionRaw = String(item.action || item.call || item.rating || "").trim();
        const text = `${actionRaw} ${detail}`;
        if (!text.trim()) return null;
        const up = /upgrade|raised|outperform|overweight|buy/i.test(text);
        const down = /downgrade|cut|underperform|underweight|sell/i.test(text);
        return {
          ticker: String(item.ticker || item.symbol || "").toUpperCase(),
          action: actionRaw || (up && !down ? "Upgrade" : down && !up ? "Downgrade" : "Analyst update"),
          detail: detail || text,
          source: String(item.source || item.broker || item.firm || "News"),
          tone: up && !down ? "good" : down && !up ? "bad" : "warn"
        };
      };

      let calls = direct.map(normalizeCall).filter(Boolean);

      if (!calls.length) {
        const news = adolExtractBigNews(scan);
        calls = news
          .filter((n) => /upgrade|downgrade|price target|raised|cut|initiated|outperform|underperform|overweight|underweight/i.test(String(n.title || "")))
          .map((n) => normalizeCall({ detail: n.title, source: n.source }))
          .filter(Boolean);
      }
      ...
      for (const c of calls) {
        const key = `${String(c.ticker || "").toUpperCase()}|${String(c.action || "").toLowerCase()}|${String(c.detail || "").toLowerCase()}`;
    ...
    function adolBigNewsSectionHtml(scan) {
      const news = adolExtractBigNews(scan);
      const calls = adolExtractAnalystCalls(scan);
      const callFilter = String(app?.adol?.newsCallFilter || "ALL").toUpperCase();
      const isUpgrade = (c) => /upgrade|raised|outperform|overweight|buy/i.test(`${c?.action || ""} ${c?.detail || ""}`);
      const isDowngrade = (c) => /downgrade|cut|underperform|underweight|sell/i.test(`${c?.action || ""} ${c?.detail || ""}`);
      const filteredCalls = calls.filter((c) => {
        if (callFilter === "UPGRADE") return isUpgrade(c) && !isDowngrade(c);
        if (callFilter === "DOWNGRADE") return isDowngrade(c) && !isUpgrade(c);
        return true;
      });
      const newsRows = news.length
        ? news.map((n) => `
            <tr>
              <td class="adol-reason"><b>${escapeHtml(n.title || "-")}</b></td>
              <td>${escapeHtml(n.source || "-")}</td>
              <td>${escapeHtml(n.tag || "NEWS")}</td>
              <td>${escapeHtml(n.impact || "-")}</td>
              <td>${n.url ? `<a href="${escapeHtml(n.url)}" target="_blank" rel="noopener">Open</a>` : "-"}</td>
            </tr>
          `).join("")
        : `<tr><td colspan="5" style="text-align:center;color:#9ec2e3;padding:14px;">No big news returned in this scan.</td></tr>`;

      const callRows = filteredCalls.length
        ? filteredCalls.map((c) => `
            <tr>
              <td><b>${escapeHtml(c.ticker || "-")}</b></td>
              <td>${adolToneChip(escapeHtml(c.action || "Analyst update"), c.tone || "warn")}</td>
              <td class="adol-reason">${escapeHtml(c.detail || "-")}</td>
              <td>${escapeHtml(c.source || "-")}</td>
            </tr>
          `).join("")
        : `<tr><td colspan="4" style="text-align:center;color:#9ec2e3;padding:14px;">No upgrades/downgrades detected in this scan.</td></tr>`;

      return `
        <div class="tile" style="margin-bottom:10px;">
          <div class="row" style="justify-content:space-between;align-items:center;gap:10px;flex-wrap:wrap;">
            <div class="k" style="margin:0;">News Filters</div>
            <div class="row" style="gap:8px;align-items:center;">
              <span style="font-size:12px;color:var(--muted2);font-weight:700;">Analyst Calls</span>
              <select onchange="window.adolSetNewsCallFilter && window.adolSetNewsCallFilter(this.value)" style="min-width:180px;">
                <option value="ALL"${callFilter === "ALL" ? " selected" : ""}>All</option>
                <option value="UPGRADE"${callFilter === "UPGRADE" ? " selected" : ""}>Upgrade only</option>
                <option value="DOWNGRADE"${callFilter === "DOWNGRADE" ? " selected" : ""}>Downgrade only</option>
              </select>
              <span class="badge">${formatNum(filteredCalls.length, 0)} shown</span>
            </div>
          </div>
        </div>
        <div class="adol-report-grid">
          <div class="tile">
            <div class="k">Big News</div>
            <div class="adol-table-wrap">
              <table class="adol-table adol-table-compact" style="min-width:860px;">
                <thead><tr><th>Headline</th><th>Source</th><th>Tag</th><th>Impact</th><th>Link</th></tr></thead>
                <tbody>${newsRows}</tbody>
              </table>
            </div>
          </div>
          <div class="tile">
            <div class="k">Upgrades / Downgrades</div>
            <div class="adol-table-wrap">
              <table class="adol-table adol-table-compact" style="min-width:760px;">
                <thead><tr><th>Ticker</th><th>Call</th><th>Detail</th><th>Source</th></tr></thead>
    ...
    function adolSetupBoardHtml(rows) {
      const list = adolApplySideFilter(Array.isArray(rows) ? rows : []);
      if (!list.length) return "";
      const ranked = [...list]
        .filter((r) => Number.isFinite(Number(r?.entry)) && Number.isFinite(Number(r?.stop)))
        .sort((a, b) => Number(b.finalAiScore || 0) - Number(a.finalAiScore || 0))
        .slice(0, 12);
      if (!ranked.length) return "";
      return `
        <div class="tile">
          <div class="k">Setup Board (Actionable Levels)</div>
          <div class="adol-table-wrap">
            <table class="adol-table adol-plan-table">
              <thead>
                <tr>
                  <th>Ticker</th>
                  <th>Action</th>
              ...
              <tbody>
                ${ranked.map((r) => {
                  const p = adolComputePlan(r);
                  if (!p) return "";
                  return `
                    <tr>
                      <td><button class="adol-ticker-btn" type="button" onclick="window.adolSelectTicker && window.adolSelectTicker('${escapeHtml(String(r.ticker || "").toUpperCase())}')">${escapeHtml(String(r.ticker || "-"))}</button></td>
                      <td>${adolToneChip(String(r.action || "-"), adolToneClassByAction(r.action, r))}</td>
                      <td>${escapeHtml(adolProjectionLabel(r))} • ${escapeHtml(p.side.toUpperCase())}</td>
                      <td>${formatUsd(p.entry)}</td>
                      <td>${formatUsd(p.stop)}</td>
                      <td>${formatUsd(p.t1)}</td>
                      <td>${formatUsd(p.t2)}</td>
                      <td>${formatUsd(p.t3)}</td>
                      <td class="adol-reason">${escapeHtml(String(r.reason || "-"))}</td>
                    </tr>
                  `;
                }).join("")}
              </tbody>
    ...
    function adolBestWorstHtml(rows) {
      const list = adolApplySideFilter(Array.isArray(rows) ? rows : [])
        .filter((r) => Number.isFinite(Number(r?.finalAiScore)));
      if (!list.length) return "";

      const actionText = (r) => String(r?.action || r?.institutionalAction || "").toLowerCase();
      const isAvoidLike = (r) => {
        const t = actionText(r);
        return t.includes("avoid") || t.includes("no trade");
      };
      const isWaitLike = (r) => {
        const t = actionText(r);
        return t.includes("wait") || t.includes("watch");
      };
      const isTradeLike = (r) => !isAvoidLike(r) && !isWaitLike(r);

      const rankedDesc = [...list].sort((a, b) => Number(b.finalAiScore || 0) - Number(a.finalAiScore || 0));
      const rankedAsc = [...list].sort((a, b) => Number(a.finalAiScore || 0) - Number(b.finalAiScore || 0));

      const bestPreferred = rankedDesc.filter((r) => isTradeLike(r));
      const bestFallback = rankedDesc.filter((r) => !bestPreferred.includes(r));
      const bestFive = [...bestPreferred, ...bestFallback].slice(0, 5);

      const worstPreferred = rankedAsc.filter((r) => isAvoidLike(r));
      const worstFallback = rankedAsc.filter((r) => !worstPreferred.includes(r));
      const worstFive = [...worstPreferred, ...worstFallback].slice(0, 5);

      const renderRows = (items, emptyText) => {
        if (!items.length) {
          return `<tr><td colspan="6" style="text-align:center;color:#9ec2e3;padding:14px;">${escapeHtml(emptyText)}</td></tr>`;
        }
        return items.map((r) => `
          <tr>
            <td><button class="adol-ticker-btn" type="button" onclick="window.adolSelectTicker && window.adolSelectTicker('${escapeHtml(String(r.ticker || "").toUpperCase())}')">${escapeHtml(String(r.ticker || "-"))}</button></td>
            <td>${adolToneChip(String(r.action || "-"), adolToneClassByAction(r.action, r))}</td>
            <td><b>${formatNum(r.finalAiScore, 0)}</b></td>
            <td>${adolToneChip(String(r.trend || "-"), adolToneClassForTrend(r.trend))}</td>
            <td>${formatUsd(r.entry)}</td>
      ...
      return `
        <div class="adol-report-grid">
          <div class="tile">
            <div class="k">Top 5 Best Trades</div>
            <div class="adol-table-wrap">
              <table class="adol-table adol-table-compact" style="min-width:760px;">
                <thead><tr><th>Ticker</th><th>Action</th><th>Final AI</th><th>Trend</th><th>Entry</th><th>Reason</th></tr></thead>
                <tbody>${renderRows(bestFive, "No best trade rows yet.")}</tbody>
              </table>
            </div>
          </div>
          <div class="tile">
            <div class="k">Top 5 Worst Trades</div>
            <div class="adol-table-wrap">
              <table class="adol-table adol-table-compact" style="min-width:760px;">
                <thead><tr><th>Ticker</th><th>Action</th><th>Final AI</th><th>Trend</th><th>Entry</th><th>Reason</th></tr></thead>
                <tbody>${renderRows(worstFive, "No worst trade rows yet.")}</tbody>
              </table>
            </div>
          </div>
        </div>
    ...
    function adolDashboardToolsHtml(rows) {
      const sideMode = String(app?.adol?.sideMode || "both").toLowerCase();
      const riskMode = String(app?.adol?.riskMode || "balanced").toLowerCase();
      const totalRows = Array.isArray(rows) ? rows.length : 0;
      const filteredRows = adolApplySideFilter(Array.isArray(rows) ? rows : []).length;
      const modeLabel = riskMode === "aggressive"
        ? "Aggressive: tighter stop, wider targets."
        : riskMode === "conservative"
          ? "Conservative: wider stop, closer targets."
          : "Balanced: default stop/target profile.";
      return `
        <div class="tile">
          <div class="k">Trade Modes</div>
          <div class="adol-dashboard-tools">
            <div class="field">
              <label for="adolRiskModeSelect">Risk Mode</label>
              <select id="adolRiskModeSelect" onchange="window.adolSetRiskMode && window.adolSetRiskMode(this.value)">
                <option value="aggressive"${riskMode === "aggressive" ? " selected" : ""}>Aggressive</option>
                <option value="balanced"${riskMode === "balanced" ? " selected" : ""}>Balanced</option>
                <option value="conservative"${riskMode === "conservative" ? " selected" : ""}>Conservative</option>
              </select>
            </div>
            <div class="field">
              <label for="adolSideModeSelect">Direction Filter</label>
              <select id="adolSideModeSelect" onchange="window.adolSetSideMode && window.adolSetSideMode(this.value)">
                <option value="both"${sideMode === "both" ? " selected" : ""}>Both (Long + Short)</option>
                <option value="long"${sideMode === "long" ? " selected" : ""}>Long Only</option>
                <option value="short"${sideMode === "short" ? " selected" : ""}>Short Only</option>
              </select>
            </div>
            <div class="field">
              <label>Live Scope</label>
              <div class="note">Showing <b>${formatNum(filteredRows, 0)}</b> of <b>${formatNum(totalRows, 0)}</b> symbols after direction filter.</div>
            </div>
          </div>
          <div class="note">Mode profile: ${escapeHtml(modeLabel)}</div>
        </div>
      `;
    }

    function adolTableControlsHtml(rows) {
      const list = Array.isArray(rows) ? rows : [];
      const state = getAdolTableState();
      const trends = ["ALL", ...Array.from(new Set(list.map((r) => String(r?.trend || "").trim()).filter(Boolean)))];
      const actions = ["ALL", ...Array.from(new Set(list.map((r) => String(r?.action || "").trim()).filter(Boolean)))];
      const mkOpts = (arr, selected) => arr.map((x) => `<option value="${escapeHtml(x)}"${x === selected ? " selected" : ""}>${escapeHtml(x)}</option>`).join("");
      return `
        <div class="adol-table-controls">
          <div class="ctrl"><label>Trend</label><select onchange="window.adolSetTableFilter && window.adolSetTableFilter('trend', this.value)">${mkOpts(trends, state.trend || "ALL")}</select></div>
          <div class="ctrl"><label>Action</label><select onchange="window.adolSetTableFilter && window.adolSetTableFilter('action', this.value)">${mkOpts(actions, state.action || "ALL")}</select></div>
          <div class="ctrl"><label>Min Bull</label><input type="number" min="0" max="100" value="${escapeHtml(String(state.minBull ?? ""))}" oninput="window.adolSetTableFilter && window.adolSetTableFilter('minBull', this.value)" /></div>
          <div class="ctrl"><label>Min Bear</label><input type="number" min="0" max="100" value="${escapeHtml(String(state.minBear ?? ""))}" oninput="window.adolSetTableFilter && window.adolSetTableFilter('minBear', this.value)" /></div>
          <div class="ctrl"><label>Min Social</label><input type="number" min="0" max="100" value="${escapeHtml(String(state.minSocial ?? ""))}" oninput="window.adolSetTableFilter && window.adolSetTableFilter('minSocial', this.value)" /></div>
          <div class="ctrl"><label>Min News</label><input type="number" min="0" max="100" value="${escapeHtml(String(state.minNews ?? ""))}" oninput="window.adolSetTableFilter && window.adolSetTableFilter('minNews', this.value)" /></div>
          <div class="ctrl"><label>Min Final AI</label><input type="number" min="0" max="100" value="${escapeHtml(String(state.minFinalAi ?? ""))}" oninput="window.adolSetTableFilter && window.adolSetTableFilter('minFinalAi', this.value)" /></div>
          <div class="ctrl"><label>Reset</label><button type="button" onclick="window.adolResetTableFilters && window.adolResetTableFilters()">Clear Filters</button></div>
        </div>
      `;
    }

    window.adolSetTableSort = function adolSetTableSort(key) {
      const state = getAdolTableState();
      if (state.sortKey === key) state.sortDir = state.sortDir === "asc" ? "desc" : "asc";
      else {
        state.sortKey = key;
        const stringKeys = new Set(["ticker", "trend", "action", "reason"]);
        state.sortDir = stringKeys.has(key) ? "asc" : "desc";
      }
      saveSettingsLocal();
      renderAdolBody();
    };

    window.adolSetTableFilter = function adolSetTableFilter(key, value) {
      const state = getAdolTableState();
      state[key] = value;
      saveSettingsLocal();
      renderAdolBody();
    ...
    window.adolSetRiskMode = function adolSetRiskMode(mode) {
      const next = String(mode || "").toLowerCase();
      if (!["aggressive", "balanced", "conservative"].includes(next)) return;
      app.adol.riskMode = next;
      saveSettingsLocal();
      renderAdolBody();
    };

    window.adolSetSideMode = function adolSetSideMode(mode) {
      const next = String(mode || "").toLowerCase();
      if (!["both", "long", "short"].includes(next)) return;
      app.adol.sideMode = next;
      saveSettingsLocal();
      renderAdolBody();
    };

    window.adolSetNewsCallFilter = function adolSetNewsCallFilter(mode) {
      const next = String(mode || "ALL").toUpperCase();
      if (!["ALL", "UPGRADE", "DOWNGRADE"].includes(next)) return;
      app.adol.newsCallFilter = next;
      saveSettingsLocal();
      renderAdolBody();
    };

    function adolApplyInstitutionalSearch(rows) {
      const list = Array.isArray(rows) ? rows : [];
      const q = String(app?.adol?.institutionalSearch || "").trim().toUpperCase();
      if (!q) return list;
      return list.filter((r) => {
        const ticker = String(r?.ticker || "").toUpperCase();
        const reason = String(r?.reason || "").toUpperCase();
        return ticker.includes(q) || reason.includes(q);
      });
    }

    window.adolSetInstitutionalSearch = function adolSetInstitutionalSearch(value) {
      app.adol.institutionalSearch = String(value || "").trim().toUpperCase();
      saveSettingsLocal();
      renderAdolBody();
    };

    window.adolClearInstitutionalSearch = function adolClearInstitutionalSearch() {
      app.adol.institutionalSearch = "";
      saveSettingsLocal();
      renderAdolBody();
    };

    window.adolResetTableFilters = function adolResetTableFilters() {
      app.adol.table = {
        ...getAdolTableState(),
        trend: "ALL",
        action: "ALL",
    ...
    function focusAdolDeepDivePanel() {
      const panel = document.getElementById("adolDeepDivePanel");
      if (!panel) return;
      panel.scrollIntoView({ behavior: "smooth", block: "start" });
      panel.classList.add("adol-focus-glow");
      setTimeout(() => panel.classList.remove("adol-focus-glow"), 1100);
    }

    window.adolSelectTicker = async function adolSelectTicker(ticker) {
      const symbol = String(ticker || "").trim().toUpperCase();
      if (!symbol) return;
      app.adol.selectedTicker = symbol;
      app.adol.page = "dashboard";
      app.adol.status = `Loading ${symbol} technical + fundamentals...`;
      app.deepDive = {
        ...(app.deepDive || {}),
        ticker: symbol,
        loading: true
      ...
      saveSettingsLocal();
      renderAdol();
      focusAdolDeepDivePanel();
      await runDeepDive(symbol, app.deepDive?.timeframe || "15M");
      try {
        await runDeepAi();
      } catch (error) {
        pushLog(`Deep AI skipped for ${symbol}: ${error?.message || "unavailable"}`);
      }
      if (app.deepDive) app.deepDive.loading = false;
      app.adol.status = `Loaded ${symbol} • Heat + Deep Dive ready`;
      renderAdol();
      focusAdolDeepDivePanel();
    };

    function normalizeAdolBaseUrl(value) {
      const raw = String(value || "").trim().replace(/\/+$/, "");
      if (!raw) return "/api/adol";
      return raw;
    }

    async function adolFetch(path, init = {}) {
      const base = normalizeAdolBaseUrl(app.adol.apiUrl || "/api/adol");
      const url = `${base}${path}`;
      const response = await fetch(url, {
        ...init,
        headers: { "Content-Type": "application/json", ...(init.headers || {}) }
    ...
    function adolTableHtml(rows) {
      const list = Array.isArray(rows) ? rows : [];
      if (!list.length) return `<div class="tile">No rows yet.</div>`;
      const view = applyAdolTableFiltersAndSort(list);
      const bodyHtml = view.length
        ? view.map((r) => `
                <tr class="${String(app?.adol?.selectedTicker || "").toUpperCase() === String(r.ticker || "").toUpperCase() ? "adol-row-selected" : ""}">
                  <td><button class="adol-ticker-btn" type="button" onclick="window.adolSelectTicker && window.adolSelectTicker('${escapeHtml(String(r.ticker || "").toUpperCase())}')">${escapeHtml(r.ticker || "-")}</button></td>
                  <td>${formatUsd(r.price)}</td>
                  <td>${formatPctSigned(r.changePct)}</td>
                  <td>${formatPctSigned(r.gapPct)}</td>
                  <td>${formatNum(r.rvol, 2)}</td>
                  <td>${adolToneChip(r.trend || "-", adolToneClassForTrend(r.trend))}</td>
                  <td>${formatNum(r.bullScore, 0)}</td>
                  <td>${formatNum(r.bearScore, 0)}</td>
                  <td>${formatNum(r.socialSentimentScore, 0)}</td>
                  <td>${formatNum(r.newsSentimentScore, 0)}</td>
                  <td><b>${formatNum(r.finalAiScore, 0)}</b></td>
                  <td>${adolToneChip(r.action || "-", adolToneClassByAction(r.action, r))}</td>
                  <td>${adolToneChip(String(r.timingSignal || "-"), adolToneClassForTiming(r.timingSignal))}</td>
                  <td class="adol-reason">${escapeHtml(r.reason || "-")}</td>
                  <td>${formatUsd(r.entry)}</td>
                  <td>${formatUsd(r.stop)}</td>
                  <td>${formatUsd(r.t1)}</td>
                  <td>${formatUsd(r.t2)}</td>
                  <td>${formatUsd(r.t3)}</td>
                </tr>
              `).join("")
        : `<tr><td colspan="19" style="text-align:center;color:#9ec2e3;padding:16px;">No matches for current filters. Try Clear Filters or lower Min Final AI.</td></tr>`;
      return `
        ${adolTableControlsHtml(list)}
        <div class="adol-table-wrap">
          <table class="adol-table">
            <thead>
              <tr>
                ${adolHeaderCell("Ticker", "ticker")}
                ${adolHeaderCell("Price", "price")}
                ${adolHeaderCell("Change %", "changePct")}
                ${adolHeaderCell("Gap %", "gapPct")}
                ${adolHeaderCell("RVOL", "rvol")}
                ${adolHeaderCell("Trend", "trend")}
                ${adolHeaderCell("Bull", "bullScore")}
                ${adolHeaderCell("Bear", "bearScore")}
                ${adolHeaderCell("Social", "socialSentimentScore")}
                ${adolHeaderCell("News", "newsSentimentScore")}
                ${adolHeaderCell("Final AI", "finalAiScore")}
                ${adolHeaderCell("Action", "action")}
                ${adolHeaderCell("Timing", "timingSignal")}
                ${adolHeaderCell("Reason", "reason")}
                ${adolHeaderCell("Entry", "entry")}
                ${adolHeaderCell("Stop", "stop")}
                ${adolHeaderCell("T1", "t1")}
                ${adolHeaderCell("T2", "t2")}
                ${adolHeaderCell("T3", "t3")}
              </tr>
            </thead>
            <tbody>
              ${bodyHtml}
    ...
    function adolCompactTableHtml(rows) {
      const list = Array.isArray(rows) ? rows : [];
      if (!list.length) return `<div class="tile">No rows yet.</div>`;
      const view = applyAdolTableFiltersAndSort(list);
      const bodyHtml = view.length
        ? view.map((r) => `
              ...
              `).join("")
        : `<tr><td colspan="10" style="text-align:center;color:#9ec2e3;padding:16px;">No matches for current filters. Try Clear Filters.</td></tr>`;
      return `
        ${adolTableControlsHtml(list)}
        <div class="adol-table-wrap">
          <table class="adol-table adol-table-compact">
            <thead>
              <tr>
                ${adolHeaderCell("Ticker", "ticker")}
                ${adolHeaderCell("Price", "price")}
    ...
    function adolOptionsFlowHtml(optionsFlow) {
      const flow = optionsFlow && typeof optionsFlow === "object" ? optionsFlow : {};
      const rows = Array.isArray(flow.rows) ? flow.rows : [];
      const provider = String(flow.provider || "disabled").toUpperCase();
      const message = String(flow.message || "Options flow is unavailable.");

      if (!rows.length) {
        return `
          <div class="tile">
            <div class="k">Options Flow</div>
      ...
      return `
        <div class="tile">
          <div class="row" style="justify-content:space-between;gap:8px;margin-bottom:8px;">
            <div class="k">Options Flow</div>
            <span class="badge">${escapeHtml(provider)} • ${rows.length} contracts</span>
          </div>
          <div style="margin-bottom:8px;">${escapeHtml(message)}</div>
          <div class="adol-table-wrap">
            <table class="adol-table" style="min-width:1500px;">
              <thead>
                <tr>
                  <th>Time (NY)</th>
                  <th>Ticker</th>
              ...
              <tbody>
                ${rows.map((r) => {
                  const side = String(r.side || "").toUpperCase() === "PUT" ? "PUT" : "CALL";
                  const scoreNum = Number(r.unusualScore || 0);
                  const scoreCls = scoreNum >= 80 ? "flow-strong" : scoreNum >= 65 ? "flow-mid" : "flow-weak";
                  return `
                    <tr>
                      <td>${escapeHtml(String(r.time || "-"))}</td>
                      <td><b>${escapeHtml(String(r.ticker || "-"))}</b></td>
                      <td><span class="flow-chip ${side === "PUT" ? "put" : "call"}">${escapeHtml(side)}</span></td>
                      <td>${escapeHtml(String(r.contract || "-"))}</td>
                      <td>${formatNum(r.dte, 0)}</td>
                      <td>${formatUsd(r.strike)}</td>
                      <td>${formatUsd(r.spot)}</td>
                      <td>${escapeHtml(String(r.bidAsk || "-"))}</td>
                      <td>${escapeHtml(String(r.fillVsSpread || "-"))}</td>
                      <td>${formatNum(r.size, 0)}</td>
                      <td>${formatUsd(r.premium)}</td>
                      <td>${formatNum(r.volume, 0)}</td>
                      <td>${formatNum(r.oi, 0)}</td>
                      <td>${formatNum(r.volOi, 2)}</td>
                      <td>${escapeHtml(String(r.chainBidAsk || "-"))}</td>
                      <td>${escapeHtml(String(r.signal || "-"))}</td>
                      <td><span class="flow-score-pill ${scoreCls}">${formatNum(scoreNum, 0)}</span></td>
                    </tr>
                  `;
                }).join("")}
              </tbody>
    ...
    function adolInstitutionalTableHtml(rows) {
      const list = Array.isArray(rows) ? rows : [];
      if (!list.length) return `<div class="tile">No institutional rows yet. Run ADOL scan.</div>`;
      return `
        <div class="adol-table-wrap">
          <table class="adol-table" style="min-width:1900px;">
            <thead>
              <tr>
                <th>Ticker</th><th>Price</th><th>Session</th><th>VWAP Control</th><th>Liquidity Event</th><th>Trap Signal</th>
                <th>Timing Signal</th><th>Top/Bottom</th><th>Institutional Pressure</th><th>Options Bias</th><th>Social Bias</th><th>Institutional Score</th><th>Setup Type</th><th>Action</th>
                <th>Entry</th><th>Stop</th><th>T1</th><th>T2</th><th>T3</th><th>Reason</th>
              </tr>
            </thead>
            <tbody>
              ${list.map((r) => `
                <tr class="${String(app?.adol?.selectedTicker || "").toUpperCase() === String(r.ticker || "").toUpperCase() ? "adol-row-selected" : ""}">
                  <td><button class="adol-ticker-btn" type="button" onclick="window.adolSelectTicker && window.adolSelectTicker('${escapeHtml(String(r.ticker || "").toUpperCase())}')">${escapeHtml(String(r.ticker || "-"))}</button></td>
                  <td>${formatUsd(r.price)}</td>
                  <td>${escapeHtml(String(r.session || app?.adol?.session || "-"))}</td>
                  <td>${adolToneChip(String(r.vwapControl || "-"), adolToneClassForVwap(r.vwapControl))}</td>
                  <td>${escapeHtml(String(r.liquidityEvent || "-"))}</td>
                  <td>${adolToneChip(String(r.trapSignal || "-"), adolToneClassForTrap(r.trapSignal))}</td>
                  <td>${adolToneChip(String(r.timingSignal || "-"), adolToneClassForTiming(r.timingSignal))}</td>
                  <td>${formatNum(r.topTimingScore, 0)} / ${formatNum(r.bottomTimingScore, 0)}</td>
                  <td>${formatNum(r.institutionalPressure, 0)}</td>
                  <td>${escapeHtml(String(r.optionsBias || "-"))}</td>
                  <td>${escapeHtml(String(r.socialBias || "-"))}</td>
                  <td><b>${formatNum(r.institutionalScore, 0)}</b></td>
                  <td>${escapeHtml(String(r.setupType || "-"))}</td>
                  <td>${adolToneChip(String(r.institutionalAction || r.action || "-"), adolToneClassByAction((r.institutionalAction || r.action || "-"), r))}</td>
                  <td>${formatUsd(r.entry)}</td>
                  <td>${formatUsd(r.stop)}</td>
                  <td>${formatUsd(r.t1)}</td>
                  <td>${formatUsd(r.t2)}</td>
    ...
    function adolLiquidityMapHtml(scan, rows, rankings) {
      const list = Array.isArray(rows) ? rows : [];
      const toNum = (v, d = 0) => {
        const n = Number(v);
        return Number.isFinite(n) ? n : d;
      };
      const pickTicker = (t) => list.find((r) => String(r?.ticker || "").toUpperCase() === String(t || "").toUpperCase()) || null;
      const macroCards = [
        { t: "SPY", r: pickTicker("SPY") },
        { t: "QQQ", r: pickTicker("QQQ") },
        { t: "DXY", r: pickTicker("DXY") },
        { t: "10Y YIELD", r: pickTicker("TNX") || pickTicker("^TNX") },
        { t: "VIX", r: pickTicker("VIX") || pickTicker("^VIX") }
      ];

      const asOfRaw = scan?.generatedAt || scan?.lastScanTime || app?.adol?.lastScanTime || "";
      const asOfDate = new Date(asOfRaw);
      const asOfText = Number.isNaN(asOfDate.getTime()) ? (asOfRaw || "-") : asOfDate.toLocaleString();
      const marketBias = String(scan?.marketBias || scan?.report?.marketBias || "Mixed");
      const riskMode = String(scan?.report?.riskMode || marketBias || "Unknown");
      const bullish = list.filter((r) => toNum(r?.bullScore) >= toNum(r?.bearScore)).length;
      const bearish = list.filter((r) => toNum(r?.bearScore) > toNum(r?.bullScore)).length;
      const sentimentPct = Math.round((bullish / Math.max(1, bullish + bearish)) * 100);
      const rvolHot = list.filter((r) => toNum(r?.rvol, 0) >= 1.5).length;

      const buckets = {};
      for (const r of list) {
        const sector = String(r?.sector || r?.industry || r?.group || "Unclassified").trim();
        if (!buckets[sector]) {
          buckets[sector] = { sector, count: 0, flowScore: 0, finalSum: 0, changeSum: 0, bullSum: 0, bearSum: 0 };
        }
        buckets[sector].count += 1;
        buckets[sector].flowScore += toNum(r?.finalAiScore, 50) + toNum(r?.bullScore, 50) - toNum(r?.bearScore, 50);
        buckets[sector].finalSum += toNum(r?.finalAiScore, 0);
        buckets[sector].changeSum += toNum(r?.changePct, 0);
        buckets[sector].bullSum += toNum(r?.bullScore, 0);
        buckets[sector].bearSum += toNum(r?.bearScore, 0);
      }

      const sectors = Object.values(buckets).map((s) => ({
        sector: s.sector,
        count: s.count,
        flow: s.flowScore / Math.max(1, s.count),
        finalAi: s.finalSum / Math.max(1, s.count),
        change: s.changeSum / Math.max(1, s.count),
        bull: s.bullSum / Math.max(1, s.count),
        bear: s.bearSum / Math.max(1, s.count)
      }));
      sectors.sort((a, b) => b.flow - a.flow);
      const topSectors = sectors.slice(0, 6);
      const bottomSectors = [...sectors].sort((a, b) => a.flow - b.flow).slice(0, 4);
      const rankSectors = sectors.slice(0, 10);

      const topBullishRows = (rankings?.topBullishMovers || []).slice(0, 6);
      const topBearishRows = (rankings?.topBearishMovers || []).slice(0, 6);
      const flowRows = (rankings?.highestRVOL || []).slice(0, 6);

      const summaryBullets = [
        `Market bias is ${marketBias} with ${sentimentPct}% bullish breadth.`,
        `${rvolHot} symbols currently have RVOL >= 1.5, signaling active rotation.`,
        topSectors[0] ? `${topSectors[0].sector} leads liquidity flow score (${formatNum(topSectors[0].flow, 1)}).` : "Awaiting sector leadership data.",
        bottomSectors[0] ? `${bottomSectors[0].sector} is the weakest flow pocket (${formatNum(bottomSectors[0].flow, 1)}).` : "No weak flow pockets detected."
      ];

      const signals = [
        { ok: topSectors.length > 0, text: "Sector leadership is visible" },
        { ok: rvolHot > 0, text: "Breadth and RVOL expansion detected" },
        { ok: String(scan?.session || "").toLowerCase().includes("regular"), text: "Regular session participation" },
        { ok: !String(scan?.status || "").toLowerCase().includes("error"), text: "Scanner status healthy" },
        { ok: (scan?.unusualOptionsFlow?.provider || "DISABLED").toUpperCase() !== "DISABLED", text: "Options flow provider active" },
        { ok: true, text: `Regime read: ${riskMode}` }
      ];

      const macroCardHtml = macroCards.map((m) => {
        const price = toNum(m?.r?.price, NaN);
        const chg = toNum(m?.r?.changePct, NaN);
        const positive = Number.isFinite(chg) ? chg >= 0 : true;
        return `
          <div class="tile" style="padding:10px;">
            <div class="k">${escapeHtml(m.t)}</div>
            <div style="font-size:24px;font-weight:900;">${Number.isFinite(price) ? formatUsd(price) : "--"}</div>
            <div style="font-size:13px;color:${positive ? "#4ade80" : "#f87171"};">${Number.isFinite(chg) ? formatPctSigned(chg) : "--"}</div>
          </div>
        `;
      }).join("");

      const heatRows = sectors.slice(0, 20).map((s, idx) => {
        const tone = s.flow >= 65 ? "green" : s.flow <= 45 ? "red" : "yellow";
        return `
          <tr>
            <td>${idx + 1}</td>
            <td>${escapeHtml(s.sector)}</td>
            <td>${adolToneChip(formatNum(s.flow, 1), tone)}</td>
            <td>${formatPctSigned(s.change)}</td>
          </tr>
        `;
      }).join("") || `<tr><td colspan="4">No sector flow data yet.</td></tr>`;

      const rankRows = rankSectors.map((s, idx) => {
        const pct = Math.max(6, Math.min(100, Math.round(s.flow)));
        const bad = s.flow < 50;
        return `
          <tr>
            <td>${idx + 1}</td>
            ...
            <td style="min-width:130px;">
              <div class="liq-bar ${bad ? "bad" : ""}"><span style="width:${pct}%;"></span></div>
            </td>
            <td>${formatNum(s.flow, 1)}</td>
          </tr>
        `;
      }).join("") || `<tr><td colspan="4">No ranking data yet.</td></tr>`;

      const flowCards = [
        ...topSectors.map((s) => ({ name: s.sector, tone: "good", flow: `FLOW: STRONG ?`, note: `Final AI ${formatNum(s.finalAi, 0)} • Bull ${formatNum(s.bull, 0)} / Bear ${formatNum(s.bear, 0)}` })),
        ...bottomSectors.slice(0, 2).map((s) => ({ name: s.sector, tone: "bad", flow: `FLOW: OUTFLOW ?`, note: `Final AI ${formatNum(s.finalAi, 0)} • Avg change ${formatPctSigned(s.change)}` }))
      ].slice(0, 6);
      const flowCardsHtml = flowCards.map((c) => `
        <article class="liq-flow-card ${c.tone}">
          <div class="name">${escapeHtml(c.name)}</div>
          <div class="meta">${escapeHtml(c.note)}</div>
          <div class="meta" style="font-weight:800;margin-top:4px;">${escapeHtml(c.flow)}</div>
        </article>
      `).join("") || `<article class="liq-flow-card"><div class="name">Awaiting scan</div><div class="meta">Run ADOL scan to build destinations.</div></article>`;

      const etfUniverse = ["SMH", "QQQ", "SPY", "XLF", "XLK", "XLE", "XLI", "XLY", "XLP", "XLV", "XLU", "IWM", "DIA"];
      const etfRows = etfUniverse.map((t) => list.find((r) => String(r?.ticker || "").toUpperCase() === t)).filter(Boolean).slice(0, 8);
      const etfTableRows = etfRows.map((r) => `
        <tr>
          <td><b>${escapeHtml(String(r.ticker || "-"))}</b></td>
          <td>${formatUsd(r.price)}</td>
          <td>${formatPctSigned(r.changePct)}</td>
          <td>${formatNum(r.finalAiScore, 0)}</td>
        </tr>
      `).join("") || `<tr><td colspan="4">No ETF rows in current scan.</td></tr>`;

      const optionsRows = Array.isArray(scan?.unusualOptionsFlow?.rows) ? scan.unusualOptionsFlow.rows.slice(0, 6) : [];
      const optionsTableRows = optionsRows.map((r) => `
        <tr>
          <td>${escapeHtml(String(r?.ticker || "-"))}</td>
          <td>${escapeHtml(String(r?.side || "-"))}</td>
          <td>${formatNum(r?.size, 0)}</td>
          <td>${formatUsd(r?.premium)}</td>
        </tr>
      `).join("") || `<tr><td colspan="4">Options flow unavailable.</td></tr>`;

      return `
        <div class="liq-map-stack">
          <div class="inst-score-grid">
            ${macroCardHtml}
            <div class="tile" style="padding:10px;">
              <div class="k">As Of</div>
              <div style="font-size:18px;font-weight:800;">${escapeHtml(asOfText)}</div>
              <div style="font-size:13px;color:var(--muted);">Session: ${escapeHtml(String(scan?.session || app?.adol?.session || "-"))}</div>
            </div>
            <div class="tile" style="padding:10px;">
              <div class="k">Market Regime</div>
              <div style="font-size:32px;font-weight:900;color:${sentimentPct >= 55 ? "#4ade80" : (sentimentPct <= 45 ? "#f87171" : "#facc15")};">${escapeHtml(riskMode)}</div>
              <div style="font-size:13px;color:var(--muted);">${escapeHtml(marketBias)} • Bull/Bear ${formatNum(bullish, 0)}/${formatNum(bearish, 0)}</div>
            </div>
          </div>

          <div class="liq-map-grid">
            <div class="tile">
              <div class="k">Sector Heat Map (Composite)</div>
              <div class="adol-table-wrap">
                <table class="adol-table adol-table-compact" style="min-width:540px;">
                  <thead><tr><th>#</th><th>Sector</th><th>Flow</th><th>Avg Change</th></tr></thead>
                  <tbody>${heatRows}</tbody>
                </table>
              </div>
            </div>

            <div class="liq-flow-hub">
              <div class="liq-flow-title">LIQUIDITY FLOW MAP</div>
              <div class="liq-flow-sub">Where smart money rotation is strongest right now</div>
              <div class="liq-flow-grid">${flowCardsHtml}</div>
              <ul class="liq-bullets">
            ...
            <div class="liq-map-stack">
              <div class="tile">
                <div class="row" style="justify-content:space-between;margin-bottom:6px;">
                  <div class="k">Liquidity Flow Ranking</div>
                  <span class="badge">Next 3-6 Months</span>
                </div>
                <div class="adol-table-wrap">
                  <table class="adol-table adol-table-compact" style="min-width:480px;">
                    <thead><tr><th>#</th><th>Theme</th><th>Flow</th><th>Score</th></tr></thead>
                    <tbody>${rankRows}</tbody>
                  </table>
                </div>
              </div>
              <div class="tile">
                <div class="k">Key Rotation Signals</div>
                <ul class="adol-checklist">
                  ${signals.map((s) => `<li>${s.ok ? "?" : "??"} ${escapeHtml(s.text)}</li>`).join("")}
                </ul>
              </div>
              <div class="tile">
                <div class="k">Institutional Sentiment</div>
                <div style="font-size:34px;font-weight:900;color:${sentimentPct >= 55 ? "#4ade80" : (sentimentPct <= 45 ? "#f87171" : "#facc15")};">${sentimentPct}%</div>
                <div style="font-size:13px;color:var(--muted);">Risk appetite: ${sentimentPct >= 55 ? "High" : sentimentPct <= 45 ? "Defensive" : "Mixed"}</div>
              </div>
            </div>
          </div>
          ...
          <div class="adol-report-grid">
            <div class="tile">
              <div class="k">Top Sector ETFs</div>
              <div class="adol-table-wrap">
                <table class="adol-table adol-table-compact" style="min-width:700px;">
                  <thead><tr><th>ETF</th><th>Price</th><th>1D%</th><th>Final AI</th></tr></thead>
                  <tbody>${etfTableRows}</tbody>
                </table>
              </div>
            </div>
            <div class="tile">
              <div class="k">Relative Strength (Scanner)</div>
              ${adolCompactTableHtml(topBullishRows)}
            </div>
            <div class="tile">
              <div class="k">Options Flow (Current Snapshot)</div>
              <div class="adol-table-wrap">
                <table class="adol-table adol-table-compact" style="min-width:700px;">
                  <thead><tr><th>Ticker</th><th>Side</th><th>Size</th><th>Premium</th></tr></thead>
                  <tbody>${optionsTableRows}</tbody>
                </table>
              </div>
            </div>
            <div class="tile">
              <div class="k">Market Summary</div>
              <ul class="adol-checklist">
                ${summaryBullets.map((b) => `<li>${escapeHtml(b)}</li>`).join("")}
                <li>Top Bullish movers: ${escapeHtml((topBullishRows.map((r) => r.ticker).join(", ") || "-"))}</li>
                <li>Top Bearish movers: ${escapeHtml((topBearishRows.map((r) => r.ticker).join(", ") || "-"))}</li>
                <li>Highest RVOL names: ${escapeHtml((flowRows.map((r) => r.ticker).join(", ") || "-"))}</li>
              </ul>
            </div>
          </div>

          ${adolBigNewsSectionHtml(scan)}
        </div>
      `;
    }

    function renderAdolPages() {
      if (!el.adolPageTabs) return;
      el.adolPageTabs.innerHTML = ADOL_PAGES.map((p) => `
        <button class="adol-page-btn ${app.adol.page === p.id ? "active" : ""}" data-adol-page="${p.id}">${escapeHtml(p.label)}</button>
      `).join("");
      el.adolPageTabs.querySelectorAll("button[data-adol-page]").forEach((btn) => {
        btn.addEventListener("click", () => {
          app.adol.page = btn.getAttribute("data-adol-page") || "dashboard";
          renderAdol();
          saveSettingsLocal();
        });
      });
    ...
    function renderAdolBody() {
      if (!el.adolPageBody) return;
      const scan = app.adol.last;
      const rows = Array.isArray(scan?.rows) ? scan.rows : [];
      const rankings = scan?.rankings || {};
      const report = scan?.report || null;

      if (!scan) {
        el.adolPageBody.innerHTML = "Run ADOL scan to load market-wide rankings and report.";
        return;
      }

      if (app.adol.page === "dashboard") {
        const filteredRows = adolApplySideFilter(rows);
        const tableRows = filteredRows.slice(0, 120);
        el.adolPageBody.innerHTML = `
          ${adolDashboardToolsHtml(rows)}
          ${adolDecisionPanelHtml(rows)}
          ${adolDeepDiveHtml(rows)}
          ${adolBigNewsSectionHtml(scan)}
          ${adolBestWorstHtml(rows)}
          ${adolSetupBoardHtml(rows)}
          <div class="tile">
            <div class="k">Market-Wide Scanner Rows</div>
            ${adolTableHtml(tableRows)}
          </div>
        `;
        return;
      }

      if (app.adol.page === "movers") {
        el.adolPageBody.innerHTML = `
          ${adolDecisionPanelHtml(rows)}
          ${adolDeepDiveHtml(rows)}
          <div class="adol-report-grid">
            <div class="tile"><div class="k">Top Bullish Movers</div>${adolCompactTableHtml((rankings.topBullishMovers || []).slice(0, 20))}</div>
            <div class="tile"><div class="k">Top Bearish Movers</div>${adolCompactTableHtml((rankings.topBearishMovers || []).slice(0, 20))}</div>
            <div class="tile"><div class="k">Biggest Gap Up</div>${adolCompactTableHtml((rankings.biggestGapUp || []).slice(0, 20))}</div>
            <div class="tile"><div class="k">Biggest Gap Down</div>${adolCompactTableHtml((rankings.biggestGapDown || []).slice(0, 20))}</div>
            <div class="tile"><div class="k">Highest RVOL</div>${adolCompactTableHtml((rankings.highestRVOL || []).slice(0, 20))}</div>
            <div class="tile"><div class="k">Strongest Trend</div>${adolCompactTableHtml((rankings.strongestTrend || []).slice(0, 20))}</div>
            <div class="tile"><div class="k">Weakest Trend</div>${adolCompactTableHtml((rankings.weakestTrend || []).slice(0, 20))}</div>
            <div class="tile"><div class="k">Session Movers Up</div>${adolCompactTableHtml((rankings.topSessionMoversUp || []).slice(0, 20))}</div>
            <div class="tile"><div class="k">Session Movers Down</div>${adolCompactTableHtml((rankings.topSessionMoversDown || []).slice(0, 20))}</div>
            <div class="tile"><div class="k">Best Risk/Reward</div>${adolCompactTableHtml((rankings.bestRiskReward || []).slice(0, 20))}</div>
          </div>
        `;
        return;
      }

      if (app.adol.page === "liquidity") {
        el.adolPageBody.innerHTML = adolLiquidityMapHtml(scan, rows, rankings);
        return;
      }

      if (app.adol.page === "institutional") {
        const ranked = [...rows].sort((a, b) => Number(b.institutionalScore || 0) - Number(a.institutionalScore || 0));
        const top = ranked;
        const searched = adolApplyInstitutionalSearch(top);
        const aPlusCount = ranked.filter((r) => Number(r.institutionalScore || 0) >= 90).length;
        const strongCount = ranked.filter((r) => Number(r.institutionalScore || 0) >= 80 && Number(r.institutionalScore || 0) < 90).length;
        const avoidCount = ranked.filter((r) => String(r.institutionalAction || "").toLowerCase().includes("avoid") || String(r.institutionalAction || "").toLowerCase().includes("no trade")).length;
        const waitCount = ranked.filter((r) => {
          const actionText = String(r.institutionalAction || r.action || "").toLowerCase();
          return actionText.includes("wait") || actionText.includes("watch");
        }).length;
        const tradeCount = ranked.filter((r) => {
          const actionText = String(r.institutionalAction || r.action || "").toLowerCase();
          return actionText && !actionText.includes("avoid") && !actionText.includes("no trade") && !actionText.includes("wait") && !actionText.includes("watch");
        }).length;
        const bullCount = ranked.filter((r) => {
          const bull = Number(r.bullScore || 0);
          const bear = Number(r.bearScore || 0);
          const vwap = String(r.vwapControl || "").toLowerCase();
          const trend = String(r.trend || "").toLowerCase();
          return bull > bear || vwap.includes("buyers in control") || trend.includes("uptrend");
        }).length;
        const bearCount = ranked.filter((r) => {
          const bull = Number(r.bullScore || 0);
          const bear = Number(r.bearScore || 0);
          const vwap = String(r.vwapControl || "").toLowerCase();
          const trend = String(r.trend || "").toLowerCase();
          return bear > bull || vwap.includes("sellers in control") || trend.includes("downtrend");
        }).length;
        const bullTraps = ranked.filter((r) => String(r.trapSignal || "").toLowerCase() === "bull trap").length;
        const bearTraps = ranked.filter((r) => String(r.trapSignal || "").toLowerCase() === "bear trap").length;

        el.adolPageBody.innerHTML = `
          ${adolDecisionPanelHtml(rows)}
          ${adolDeepDiveHtml(rows)}
          <div class="inst-score-grid" style="margin-bottom:10px;">
            <div class="inst-score-card good"><div class="k">A+ Institutional</div><div style="font-size:28px;font-weight:900;">${formatNum(aPlusCount, 0)}</div></div>
            <div class="inst-score-card warn"><div class="k">Strong Setup</div><div style="font-size:28px;font-weight:900;">${formatNum(strongCount, 0)}</div></div>
            <div class="inst-score-card bad"><div class="k">Avoid / No Trade</div><div style="font-size:28px;font-weight:900;">${formatNum(avoidCount, 0)}</div></div>
            <div class="inst-score-card cool"><div class="k">Trade</div><div style="font-size:24px;font-weight:900;">${formatNum(tradeCount, 0)}</div></div>
            <div class="inst-score-card cool"><div class="k">Wait / Watch</div><div style="font-size:24px;font-weight:900;">${formatNum(waitCount, 0)}</div></div>
            <div class="inst-score-card good"><div class="k">Bulls</div><div style="font-size:24px;font-weight:900;">${formatNum(bullCount, 0)}</div></div>
            <div class="inst-score-card bad"><div class="k">Bears</div><div style="font-size:24px;font-weight:900;">${formatNum(bearCount, 0)}</div></div>
            <div class="inst-score-card good"><div class="k">Bull Traps</div><div style="font-size:24px;font-weight:900;">${formatNum(bullTraps, 0)}</div></div>
            <div class="inst-score-card bad"><div class="k">Bear Traps</div><div style="font-size:24px;font-weight:900;">${formatNum(bearTraps, 0)}</div></div>
            <div class="inst-score-card warn"><div class="k">Session</div><div style="font-size:20px;font-weight:900;">${escapeHtml(String(scan?.session || app?.adol?.session || "-"))}</div></div>
          </div>
          <div class="tile">
            <div class="row" style="justify-content:space-between;gap:8px;align-items:flex-end;flex-wrap:wrap;">
              <div class="field" style="min-width:260px;flex:1;">
                <label>Search Ticker</label>
                <input type="text" value="${escapeHtml(String(app?.adol?.institutionalSearch || ""))}" placeholder="ex: TSLA, AMD, NVDA" oninput="window.adolSetInstitutionalSearch && window.adolSetInstitutionalSearch(this.value)" />
              </div>
              <div class="field" style="min-width:180px;">
                <label>Rows</label>
                <div class="note">Showing <b>${formatNum(searched.length, 0)}</b> of <b>${formatNum(top.length, 0)}</b></div>
              </div>
              <button type="button" class="btn" onclick="window.adolClearInstitutionalSearch && window.adolClearInstitutionalSearch()">Clear Search</button>
            </div>
          </div>
          ${adolInstitutionalTableHtml(searched)}
        `;
        return;
      }

      if (app.adol.page === "optionsflow") {
        el.adolPageBody.innerHTML = adolOptionsFlowHtml(scan?.unusualOptionsFlow || null);
        return;
      }

      if (app.adol.page === "social") {
        const socialRows = rows
          .map((r) => ({
            ticker: r.ticker,
            social: r.socialSentimentScore,
            news: r.newsSentimentScore,
            velocity: r?.sentiment?.mentionsVelocity || 1,
            action: r.action,
            reason: r.reason
          }))
        ...
        el.adolPageBody.innerHTML = `
          <div class="tile">
            <div class="k">Social Sentiment Leaders</div>
            <div class="adol-table-wrap">
              <table class="adol-table" style="min-width:880px;">
                <thead><tr><th>Ticker</th><th>Social Sentiment</th><th>News Sentiment</th><th>Mentions Velocity</th><th>Action</th><th>Reason</th></tr></thead>
                <tbody>
                  ${socialRows.map((r) => `<tr><td><b>${escapeHtml(r.ticker)}</b></td><td>${formatNum(r.social, 0)}</td><td>${formatNum(r.news, 0)}</td><td>${formatNum(r.velocity, 2)}</td><td>${escapeHtml(r.action)}</td><td class="adol-reason">${escapeHtml(r.reason)}</td></tr>`).join("")}
                </tbody>
              </table>
            </div>
          </div>
          <div class="tile">
            ...
            <div>${escapeHtml(scan?.socialStatus?.x || "X sentiment disabled.")}</div>
            <div>${escapeHtml(scan?.socialStatus?.reddit || "Reddit sentiment disabled.")}</div>
            <div>${escapeHtml(scan?.socialStatus?.stocktwits || "StockTwits sentiment disabled.")}</div>
            <div>${escapeHtml(scan?.socialStatus?.news || "News sentiment disabled.")}</div>
          </div>
        `;
        return;
      }

      if (app.adol.page === "watchlist") {
        const watch = parseAdolWatchlist(app.adol.watchlistText);
        const watchRows = rows.filter((r) => watch.includes(r.ticker));
        el.adolPageBody.innerHTML = `
          <div class="tile"><div class="k">Watchlist Symbols</div>${escapeHtml(watch.join(", ") || "-")}</div>
          ${adolTableHtml(watchRows)}
        `;
        return;
      }

      if (app.adol.page === "report") {
        const inst = report?.institutionalModeSummary || {};
        const perf = report?.performance || app.adol.performance || {};
        const weekly = report?.weeklyJournalReport || {};
        const risk = scan?.riskManager || app.adol.riskManager || {};
        el.adolPageBody.innerHTML = `
          <div class="adol-report-grid">
            <div class="tile"><div class="k">Market Bias</div><div style="font-size:24px;font-weight:800;">${escapeHtml(report?.marketBias || "-")}</div></div>
            <div class="tile"><div class="k">Risk Mode</div><div style="font-size:24px;font-weight:800;">${escapeHtml(report?.riskMode || "-")}</div></div>
            <div class="tile"><div class="k">Counts</div><div>Total: ${formatNum(report?.counts?.total, 0)} | Longs: ${formatNum(report?.counts?.longs, 0)} | Shorts: ${formatNum(report?.counts?.shorts, 0)} | A+: ${formatNum(report?.counts?.aPlus, 0)}</div></div>
            <div class="tile"><div class="k">Session</div><div>${escapeHtml(report?.session || "-")} • ${escapeHtml(report?.generatedAt || "-")}</div></div>
            <div class="tile"><div class="k">Risk Manager</div><div>Fixed Risk: ${formatUsd(risk?.fixedRiskUsd || 0)} | Daily Max Loss: ${formatNum(risk?.dailyMaxLossR || 0, 2)}R</div><div>Daily Realized: ${formatNum(risk?.dailyRealizedR || 0, 2)}R | ${risk?.canTrade ? "Trading enabled" : "Trading paused"}</div></div>
            <div class="tile"><div class="k">Performance</div><div>Win rate: ${formatNum(perf?.winRate || 0, 1)}% | PF: ${formatNum(perf?.profitFactor || 0, 2)}</div><div>Avg win/loss: ${formatNum(perf?.avgWinR || 0, 2)}R / ${formatNum(perf?.avgLossR || 0, 2)}R</div><div>Best setup: ${escapeHtml(perf?.bestSetupType || "N/A")}</div></div>
            <div class="tile"><div class="k">Weekly Journal</div><div>Closed: ${formatNum(weekly?.tradesClosed || 0, 0)} | Win rate: ${formatNum(weekly?.winRate || 0, 1)}% | Net: ${formatNum(weekly?.netR || 0, 2)}R</div><div>Wins/Losses: ${formatNum(weekly?.wins || 0, 0)} / ${formatNum(weekly?.losses || 0, 0)}</div></div>
          </div>
          <div class="tile">
            <div class="k">Institutional Mode Summary</div>
            <div>Buyers in control: ${escapeHtml((inst.buyersInControl || []).join(", ") || "-")}</div>
            <div>Sellers in control: ${escapeHtml((inst.sellersInControl || []).join(", ") || "-")}</div>
            <div>Liquidity grabs: ${escapeHtml((inst.liquidityGrabsDetected || []).map((x) => x.ticker || "").join(", ") || "-")}</div>
            <div>Bull traps: ${escapeHtml((inst.bullTrapsDetected || []).join(", ") || "-")}</div>
            <div>Bear traps: ${escapeHtml((inst.bearTrapsDetected || []).join(", ") || "-")}</div>
            <div>Quiet accumulation: ${escapeHtml((inst.quietAccumulationNames || []).join(", ") || "-")}</div>
            <div>Hype trap names: ${escapeHtml((inst.hypeTrapNames || []).join(", ") || "-")}</div>
            <div>Best A+ institutional: ${escapeHtml(inst?.bestAPlusInstitutionalSetup?.ticker || "-")} ${inst?.bestAPlusInstitutionalSetup?.institutionalScore ? `(${formatNum(inst.bestAPlusInstitutionalSetup.institutionalScore, 0)})` : ""}</div>
            <div>What to avoid: ${escapeHtml((inst.whatToAvoid || []).map((x) => x.ticker || "").join(", ") || "-")}</div>
          </div>
          <div class="tile"><div class="k">Macro Alerts</div><ul>${(report?.macroAlerts || []).map((m) => `<li><b>${escapeHtml(m.trigger || "-")}:</b> ${escapeHtml(m.detail || "-")} (${escapeHtml(m.action || "-")})</li>`).join("") || "<li>No macro alerts in this scan.</li>"}</ul></div>
          <div class="tile"><div class="k">Top Weekly Money Makers</div><ul>${(weekly?.topMoneyMakers || []).map((m) => `<li>${escapeHtml(m.tag)} • trades ${formatNum(m.trades || 0, 0)} • net ${formatNum(m.netR || 0, 2)}R</li>`).join("") || "<li>No closed journal trades yet.</li>"}</ul></div>
          <div class="tile"><div class="k">Final Action Plan</div><ul>${(report?.finalActionPlan || []).map((x) => `<li>${escapeHtml(x)}</li>`).join("")}</ul></div>
          <div class="tile"><div class="k">Best Trade Setups</div>${adolTableHtml((report?.bestTradeSetups || []).slice(0, 15))}</div>
          <div class="tile"><div class="k">Avoid</div>${adolTableHtml((report?.avoidList || []).slice(0, 20))}</div>
        `;
        return;
      }

      if (app.adol.page === "settings") {
        const s = scan?.socialStatus || {};
        const optionsFlow = scan?.unusualOptionsFlow || {};
        const optionsProvider = String(optionsFlow.provider || "disabled").toUpperCase();
        const optionsMessage = String(optionsFlow.message || "Options flow disabled.");
        const darkPoolEnabled = optionsProvider !== "DISABLED";
        el.adolPageBody.innerHTML = `
          <div class="tile"><div class="k">Schedule</div><div><code>*/5 7-20 * * 1-5</code> • <code>America/New_York</code></div></div>
          <div class="tile"><div class="k">Sessions</div><div>Premarket: 07:00-09:29 | Regular: 09:30-16:00 | After Hours: 16:01-20:00</div></div>
          <div class="tile"><div class="k">API Availability</div><div>${escapeHtml(s.x || "X sentiment disabled.")}</div><div>${escapeHtml(s.reddit || "Reddit sentiment disabled.")}</div><div>${escapeHtml(s.stocktwits || "StockTwits sentiment disabled.")}</div><div>${escapeHtml(s.news || "News sentiment disabled.")}</div></div>
          <div class="tile"><div class="k">Options Flow Provider</div><div>${escapeHtml(optionsProvider)}</div><div>${escapeHtml(optionsMessage)}</div></div>
          <div class="tile"><div class="k">Dark Pool / Block Trade</div><div>${darkPoolEnabled ? "Provider-linked proxy enabled." : "Dark pool data disabled"}</div><div>Supported providers: Unusual Whales, Tradier, Polygon, Intrinio, Benzinga Pro.</div></div>
          <div class="tile"><div class="k">Alert Rules</div><div>A+ Institutional Setup • Liquidity sweep + reclaim • VWAP reclaim with RVOL > 2 • Major bull/bear trap • Institutional Score >= 90</div></div>
          <div class="tile"><div class="k">No-Trade Filters</div><div>Open-buffer, low-volume, and wide-spread filters are active before any alert is sent.</div><div>Session-edge mode: Premarket (gap+RVOL), Open (reclaim/sweep), Midday (reduced size), Power Hour (momentum continuation).</div></div>
        `;
      }
    }

    function renderAdol() {
      if (el.adolWatchlistInput) el.adolWatchlistInput.value = app.adol.watchlistText || "";
      if (el.adolSessionBadge) el.adolSessionBadge.textContent = app.adol.status || app.adol.session || "Idle";
      if (el.adolMetaSession) el.adolMetaSession.textContent = app.adol.session || "-";
      if (el.adolMetaMarket) el.adolMetaMarket.textContent = app.adol.marketStatus || "-";
      if (el.adolMetaLast) el.adolMetaLast.textContent = app.adol.lastScanTime || "-";
      if (el.adolMetaNext) el.adolMetaNext.textContent = app.adol.nextScanTime || "-";
      renderAdolPages();
      renderAdolBody();
    }

    async function refreshAdolStatus() {
      app.adol.status = "Loading status...";
      renderAdol();
      try {
        const status = await adolFetch("/status");
        app.adol.session = status?.session?.label || "Unknown";
        app.adol.marketStatus = status?.marketStatus || status?.session?.marketStatus || "Unknown";
        app.adol.lastScanTime = status?.scheduler?.lastRunAt || app.adol.lastScanTime || "-";
        app.adol.nextScanTime = status?.scheduler?.nextRunAt || "-";
        app.adol.performance = status?.performance || app.adol.performance;
        app.adol.riskManager = status?.riskManager || app.adol.riskManager;
        app.adol.status = `Session: ${app.adol.session}`;
      } catch (error) {
        app.adol.status = "Status error";
        app.adol.marketStatus = "Unknown";
        app.adol.session = "Unknown";
        app.adol.lastScanTime = `Failed: ${error?.message || "Unknown error"}`;
      }
      if (!app.adol.last || !Array.isArray(app.adol.last?.rows) || !app.adol.last.rows.length) {
        await loadAdolLastScan({ silent: true });
      }
      renderAdol();
    }

    async function loadAdolLastScan({ silent = false } = {}) {
      try {
        const data = await adolFetch("/last");
        const rows = Array.isArray(data?.rows) ? data.rows : [];
        if (!rows.length) return false;
        app.adol.last = data;
        app.adol.session = data?.session || app.adol.session || "Unknown";
        app.adol.marketStatus = data?.marketStatus || app.adol.marketStatus || "Unknown";
        app.adol.lastScanTime = data?.lastScanTime || data?.generatedAt || app.adol.lastScanTime || "-";
        app.adol.nextScanTime = data?.nextScanTime || app.adol.nextScanTime || "-";
        app.adol.performance = data?.performance || app.adol.performance;
        app.adol.riskManager = data?.riskManager || app.adol.riskManager;
        const hasSelected = rows.some((r) => String(r?.ticker || "").toUpperCase() === String(app.adol.selectedTicker || "").toUpperCase());
        if (!hasSelected) app.adol.selectedTicker = String(rows?.[0]?.ticker || "").toUpperCase();
        if (!silent) app.adol.status = `Loaded last scan • ${rows.length} symbols`;
        return true;
      } catch {
        return false;
      }
    ...
    async function runAdolScan() {
      if (adolScanBusy) {
        app.adol.status = "Scan in progress...";
        renderAdol();
        setTimeout(() => { refreshAdolStatus(); }, 1500);
        return;
      }
      adolScanBusy = true;
      if (el.adolScanBtn) el.adolScanBtn.disabled = true;
      app.adol.watchlistText = String(el.adolWatchlistInput?.value || app.adol.watchlistText || "");
      const watchlist = parseAdolWatchlist(app.adol.watchlistText);
      app.adol.status = "Running ADOL scan...";
      renderAdol();
      try {
        await adolFetch("/settings", {
          method: "POST",
          body: JSON.stringify({ watchlist })
        });
        const data = await adolFetch("/scan", {
          method: "POST",
          body: JSON.stringify({ watchlist })
        });
        if (data?.busy) {
          app.adol.status = "Scan in progress...";
          app.adol.lastScanTime = "Scan already running (auto-refreshing status...)";
          setTimeout(() => { refreshAdolStatus(); }, 1500);
          return;
        }
        app.adol.last = data;
        app.adol.session = data?.session || "Unknown";
        app.adol.marketStatus = data?.marketStatus || "Unknown";
        app.adol.lastScanTime = data?.lastScanTime || data?.generatedAt || new Date().toISOString();
        app.adol.nextScanTime = data?.nextScanTime || "-";
        app.adol.performance = data?.performance || app.adol.performance;
        app.adol.riskManager = data?.riskManager || app.adol.riskManager;
        const rows = Array.isArray(data?.rows) ? data.rows : [];
        const hasSelected = rows.some((r) => String(r?.ticker || "").toUpperCase() === String(app.adol.selectedTicker || "").toUpperCase());
        if (!hasSelected) app.adol.selectedTicker = String(rows?.[0]?.ticker || "").toUpperCase();
        const sent = Number(data?.alerts?.sent || 0);
        app.adol.status = `Completed • ${data?.rows?.length || 0} symbols • alerts ${sent}`;
        pushLog(`ADOL scan complete (${data?.rows?.length || 0} rows)`);
      } catch (error) {
        ...
        if (msg.toLowerCase().includes("already running")) {
          ...
          app.adol.status = "Scan failed";
          app.adol.lastScanTime = `Error: ${msg}`;
        }
      } finally {
        adolScanBusy = false;
        if (el.adolScanBtn) el.adolScanBtn.disabled = false;
      }
      saveSettingsLocal();
      renderAdol();
    }
    ...
    function renderNewsHub() {
      if (!el.newsHubList) return;
      const items = Array.isArray(app.newsItems) ? app.newsItems : [];
      if (!items.length) {
        el.newsHubList.innerHTML = `<div class="tile">No news available yet. Click refresh or run a scan.</div>`;
        return;
      }
      el.newsHubList.innerHTML = items.slice(0, 40).map((n) => `
        <div class="news-item">
          <div class="badges">
            <span class="badge">${escapeHtml(String(n.tag || "NEWS"))}</span>
            <span class="badge">${escapeHtml(String(n.impact || "Medium"))}</span>
          </div>
          <div style="font-weight:700;">${escapeHtml(String(n.title || "-"))}</div>
          <div style="font-size:12px;color:var(--muted2);">${escapeHtml(String(n.source || "Source"))}</div>
        </div>
      `).join("");
    }
    function renderLiquidityHub() {
      if (!el.liquidityHubBody) return;
      const scan = app?.adol?.last;
      const rows = Array.isArray(scan?.rows) ? scan.rows : [];
      if (!rows.length) {
        el.liquidityHubBody.innerHTML = `
          <div class="tile">
            Liquidity map will appear after ADOL scan data loads.
            <div style="margin-top:10px;">Open ADOL22 and click <b>Run ADOL Scan</b> or <b>Refresh Status</b>.</div>
          </div>
        `;
        return;
      }
      const rankings = adolBuildTopMovers(rows);
      el.liquidityHubBody.innerHTML = adolLiquidityMapHtml(scan, rows, rankings);
    }
...
${model.symbol} is ${model.trend.toLowerCase()} with ${model.momentum.toLowerCase()} momentum. Confirm reclaim/reject behavior at ${formatMoney(model.gammaFlip)} before full size.</div>
    ...
    async function applySymbolSearch(openDeepDive = false) {
      ...
      if (openDeepDive) {
        ...
        pushLog(`Loaded ${symbol} + scan report updated`);
      } else {
    ...
    async function runDeepAi() {
      ...
      const prompt = [
        ...
        `Ticker: ${symbol}`,
        `Daily/4H/1H/15m context required. Use best inference with provided data and set missing fields to UNAVAILABLE/0 without omitting keys.`,
        `Fundamentals snapshot: market cap ${fund.marketCap || "N/A"}, PE ${fund.pe ?? "N/A"}, EPS ${fund.eps ?? "N/A"}, earnings ${fund.earningsDate || "N/A"}, sector ${fund.sector || "N/A"}, industry ${fund.industry || "N/A"}.`,
        `Technicals snapshot: trend ${tech.trend || "N/A"}, momentum ${tech.momentum || "N/A"}, close ${tech.close ?? "N/A"}, EMA9 ${tech.ema9 ?? "N/A"}, EMA21 ${tech.ema21 ?? "N/A"}, EMA200 ${tech.ema200 ?? "N/A"}, VWAP ${tech.vwap ?? "N/A"}, RSI ${tech.rsi ?? "N/A"}, MACD ${tech.macdLine ?? "N/A"}.`,
        `Scoring rules: penalize weak volume/weak close/overhead resistance; reward multi-timeframe trend alignment, RVOL strength, and institutional confirmation.`,
        `Allowed setup_classification: STRONG BUY, BUY PULLBACK, WAIT, OVEREXTENDED, SELL / EXIT, SHORT SETUP.`,
        `Allowed heat_risk: HEALTHY, OVEREXTENDED, CLIMACTIC.`,
        `Allowed action.decision: ENTER NOW, WAIT FOR PULLBACK, TAKE PROFITS, AVOID.`,
        `Return exactly this JSON shape with all keys present:`,
        `{`,
        `  "ticker":"", "timestamp_utc":"",`,
        `  "overall_bias":{"bias":"BULLISH | BEARISH | NEUTRAL","color_tag":"GREEN | RED | YELLOW","summary":""},`,
        `  "ai_confidence_score":0, "setup_classification":"",`,
        `  "entry_zone":{"min":0,"max":0,"note":""},`,
        `  "stop_loss":{"price":0,"basis":"ATR | VWAP | structure"},`,
        `  "targets":{"t1":0,"t2":0,"t3":0},`,
        `  "risk_reward":{"to_t1":0,"to_t2":0,"to_t3":0},`,
        `  "best_timeframe":"1D | 4H | 1H | 15m",`,
        `  "trade_quality_grade":"A+ | A | B | C | D | F",`,
        `  "institutional_activity_analysis":{"options_flow_bias":"BULLISH | BEARISH | NEUTRAL | UNAVAILABLE","dark_pool_bias":"ACCUMULATION | DISTRIBUTION | NEUTRAL | UNAVAILABLE","smart_money_accumulation":0,"notes":""},`,
        `  "momentum_strength":{"score":0,"state":"STRONG | MODERATE | WEAK","notes":""},`,
        `  "heat_risk":{"state":"HEALTHY | OVEREXTENDED | CLIMACTIC","color_tag":"GREEN | ORANGE | RED","notes":""},`,
        `  "probabilities":{"trend_continuation_probability":0,"fake_breakout_probability":0,"squeeze_potential":0,"trap_risk":0,"exhaustion_risk":0},`,
        `  "levels":{"support":[0],"resistance":[0],"liquidity_zones":[0]},`,
        `  "action":{"decision":"ENTER NOW | WAIT FOR PULLBACK | TAKE PROFITS | AVOID","color_tag":"GREEN | YELLOW | ORANGE | RED","reason":""},`,
        `  "execution_plan":{"one_line_plan":"","invalidation_trigger":""}`,
        `}`
      ].join("\n");
    ...
    function renderAll() { renderSummary(); renderOverview(); renderHeat(); renderWatch(); renderMacro(); renderNews(); renderNewsHub(); renderLiquidityHub(); renderLiveTv(); renderScanner(); gdRenderAll(); renderAdol(); renderPrayerTimes(); renderMetrics(); renderChart(); renderLog(); renderWebV3(); renderConfig(); renderDeepDive(); renderProTrader(); applyTheme(); }
    ...
    function bind() {
      ...
      if (el.scannerSaveTelegramBtn) {
        el.scannerSaveTelegramBtn.addEventListener("click", async () => {
          await saveScannerTelegramConfig();
        });
      }
      if (el.scannerTelegramTestBtn) {
        el.scannerTelegramTestBtn.addEventListener("click", async () => {
          await testScannerTelegram();
        });
      }
      if (el.gdCalcBtn) {
        el.gdCalcBtn.addEventListener("click", () => {
          gdCalculateAndRender();
        });
      }
      if (el.gdSaveBtn) {
        el.gdSaveBtn.addEventListener("click", async () => {
          await gdSaveDeal();
        });
      }
      if (el.gdSendAlertBtn) {
        el.gdSendAlertBtn.addEventListener("click", async () => {
          try {
            await gdSendCurrentAlert(true);
          } catch (error) {
            gdSetStatus(`Telegram failed: ${error?.message || "Unknown error"}`);
          }
        });
      }
      if (el.gdSaveTelegramBtn) {
        el.gdSaveTelegramBtn.addEventListener("click", async () => {
          app.greatDeals.telegramToken = String(el.gdTelegramToken?.value || "").trim();
          app.greatDeals.telegramChatId = String(el.gdTelegramChatId?.value || "").trim();
          saveSettingsLocal();
          try {
            const res = await fetch("/api/great-deals/telegram/config", {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({
                telegram_bot_token: app.greatDeals.telegramToken,
                telegram_chat_id: app.greatDeals.telegramChatId
              })
            });
            const data = await res.json().catch(() => ({}));
            if (!res.ok || data?.ok === false) {
              gdSetStatus(`Telegram save failed: ${data?.error || `HTTP ${res.status}`}`);
              return;
            }
            gdSetStatus("Telegram settings saved.");
          } catch (error) {
            gdSetStatus(`Telegram save failed: ${error?.message || "Unknown error"}`);
          }
        });
      }
      if (el.gdTestTelegramBtn) {
        el.gdTestTelegramBtn.addEventListener("click", async () => {
          try {
            app.greatDeals.telegramToken = String(el.gdTelegramToken?.value || "").trim();
            app.greatDeals.telegramChatId = String(el.gdTelegramChatId?.value || "").trim();
            saveSettingsLocal();
            const res = await fetch("/api/great-deals/telegram/test", {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({
                telegram_bot_token: app.greatDeals.telegramToken,
            ...
            const data = await res.json().catch(() => ({}));
            if (!res.ok || data?.ok === false) {
              gdSetStatus(`Telegram test failed: ${data?.error || `HTTP ${res.status}`}`);
              return;
            }
            gdSetStatus("Telegram test sent.");
          } catch (error) {
            gdSetStatus(`Telegram test failed: ${error?.message || "Unknown error"}`);
          }
        });
      }
      if (el.gdParseTextBtn) {
        el.gdParseTextBtn.addEventListener("click", () => {
          gdParseTextScanner();
        });
      }
      if (el.gdRunScanBtn) {
        el.gdRunScanBtn.addEventListener("click", async () => {
          await gdRunHybridScan();
        });
      }
      if (el.gdOpenSearchesBtn) {
        el.gdOpenSearchesBtn.addEventListener("click", () => {
          gdOpenSearchLaunchers();
        });
      }
      if (el.gdImportTopBtn) {
        el.gdImportTopBtn.addEventListener("click", () => {
          gdImportTopCandidate();
        });
      }
      if (el.gdSendNewCandidatesBtn) {
        el.gdSendNewCandidatesBtn.addEventListener("click", async () => {
          try {
            const result = await gdSendCandidatesToTelegram(app.greatDeals.hybrid.candidates || [], 10);
            gdSetStatus(result.sent > 0 ? `Sent ${result.sent} new candidate(s) to Telegram.` : "No new candidates to send.");
          } catch (error) {
            gdSetStatus(`Telegram failed: ${error?.message || "Unknown error"}`);
          }
        });
      }
      if (el.gdCandidatesBody) {
        el.gdCandidatesBody.addEventListener("click", (event) => {
          const importBtn = event.target instanceof Element ? event.target.closest("[data-gd-cand-import]") : null;
          if (importBtn) {
            const id = String(importBtn.getAttribute("data-gd-cand-import") || "");
            if (id) gdImportCandidate(id);
            return;
          }
          const saveBtn = event.target instanceof Element ? event.target.closest("[data-gd-cand-save]") : null;
          if (saveBtn) {
            const id = String(saveBtn.getAttribute("data-gd-cand-save") || "");
            if (id) gdSaveCandidate(id, "new");
          }
        });
      }
      if (el.gdCsvImportBtn) {
        el.gdCsvImportBtn.addEventListener("click", async () => {
          await gdImportCsvFile();
        });
      }
      if (el.gdAutoAlert) {
        el.gdAutoAlert.addEventListener("change", () => {
          app.greatDeals.autoAlert = !!el.gdAutoAlert.checked;
          saveSettingsLocal();
        });
      }
      if (el.gdAutoScanEvery5) {
        el.gdAutoScanEvery5.addEventListener("change", () => {
          app.greatDeals.hybrid.autoScanEnabled = !!el.gdAutoScanEvery5.checked;
          saveSettingsLocal();
          gdRenderAutoScanMeta();
          gdStartAutoScanScheduler();
          gdSetStatus(app.greatDeals.hybrid.autoScanEnabled ? "Auto scan enabled (every 5 min)." : "Auto scan disabled.");
        });
      }
      if (el.gdAutoSendOnScan) {
        el.gdAutoSendOnScan.addEventListener("change", () => {
          app.greatDeals.hybrid.autoSendOnScan = !!el.gdAutoSendOnScan.checked;
          saveSettingsLocal();
          gdSetStatus(app.greatDeals.hybrid.autoSendOnScan ? "Auto-send enabled (A/A+ only)." : "Auto-send disabled.");
        });
      }
      if (el.gdSearch) {
        el.gdSearch.addEventListener("input", () => {
          app.greatDeals.filters.search = String(el.gdSearch.value || "");
          gdRenderDealsTable();
          saveSettingsLocal();
        });
      }
      if (el.gdFilterGrade) {
        el.gdFilterGrade.addEventListener("change", () => {
          app.greatDeals.filters.grade = String(el.gdFilterGrade.value || "ALL");
          gdRenderDealsTable();
          saveSettingsLocal();
        });
      }
      if (el.gdSortBy) {
        el.gdSortBy.addEventListener("change", () => {
          app.greatDeals.filters.sortBy = String(el.gdSortBy.value || "profit");
          gdRenderDealsTable();
          saveSettingsLocal();
        });
      }
      if (el.gdSortDir) {
        el.gdSortDir.addEventListener("change", () => {
          app.greatDeals.filters.sortDir = String(el.gdSortDir.value || "desc");
          gdRenderDealsTable();
          saveSettingsLocal();
        });
      }
      if (el.gdClearDealsBtn) {
        el.gdClearDealsBtn.addEventListener("click", () => {
          app.greatDeals.deals = [];
          app.greatDeals.editingId = "";
          saveSettingsLocal();
          gdRenderDealsTable();
          gdSetStatus("All deals cleared.");
        });
      }
      if (el.gdDealsBody) {
        el.gdDealsBody.addEventListener("click", async (event) => {
          const target = event.target instanceof Element ? event.target.closest("[data-gd-action]") : null;
          if (!target) return;
          const action = String(target.getAttribute("data-gd-action") || "");
          const id = String(target.getAttribute("data-gd-id") || "");
          if (!id) return;
          if (action === "edit") gdLoadDealToForm(id);
          if (action === "delete") gdDeleteDeal(id);
          if (action === "alert") {
            try {
              await gdAlertDeal(id);
            } catch (error) {
              gdSetStatus(`Alert failed: ${error?.message || "Unknown error"}`);
            }
          }
          if (action === "contacted") await gdUpdateDealStatus(id, "contacted");
          if (action === "bought") await gdUpdateDealStatus(id, "bought");
          if (action === "sold") await gdUpdateDealStatus(id, "sold");
          if (action === "skip") await gdUpdateDealStatus(id, "skip");
        });
      }
      Array.from([el.gdTitle, el.gdCategory, el.gdBuyPrice, el.gdSoldPrice, el.gdShipping, el.gdFeePct, el.gdCondition, el.gdDemand, el.gdRisk, el.gdEase, el.gdLink, el.gdNotes]).forEach((node) => {
        if (!node) return;
      ...
      Array.from([el.gdScanQuery, el.gdScanMinPrice, el.gdScanMaxPrice, el.gdScanMinProfit, el.gdScanDistance, el.gdScanCity, el.gdSourceEbay, el.gdSourceCraigslist, el.gdSourceFacebook, el.gdSourceSerp]).forEach((node) => {
        if (!node) return;
        node.addEventListener("change", () => {
          app.greatDeals.hybrid.query = String(el.gdScanQuery?.value || "");
          app.greatDeals.hybrid.minPrice = gdNum(el.gdScanMinPrice?.value || 0);
          app.greatDeals.hybrid.maxPrice = gdNum(el.gdScanMaxPrice?.value || 0);
          app.greatDeals.hybrid.minProfit = gdNum(el.gdScanMinProfit?.value || 0);
          app.greatDeals.hybrid.distance = gdNum(el.gdScanDistance?.value || 0);
          app.greatDeals.hybrid.city = String(el.gdScanCity?.value || "cincinnati");
          app.greatDeals.hybrid.sourceEbay = !!el.gdSourceEbay?.checked;
          app.greatDeals.hybrid.sourceCraigslist = !!el.gdSourceCraigslist?.checked;
          app.greatDeals.hybrid.sourceFacebook = !!el.gdSourceFacebook?.checked;
          app.greatDeals.hybrid.sourceSerp = !!el.gdSourceSerp?.checked;
          saveSettingsLocal();
        });
      });
      Array.from(document.querySelectorAll(".gdChecklistItem")).forEach((node, index) => {
        node.addEventListener("change", () => {
          app.greatDeals.checklist = {
            comps: !!document.querySelectorAll(".gdChecklistItem")[0]?.checked,
            condition: !!document.querySelectorAll(".gdChecklistItem")[1]?.checked,
            shipping: !!document.querySelectorAll(".gdChecklistItem")[2]?.checked,
            risk: !!document.querySelectorAll(".gdChecklistItem")[3]?.checked,
            negotiate: !!document.querySelectorAll(".gdChecklistItem")[4]?.checked,
            pickup: !!document.querySelectorAll(".gdChecklistItem")[5]?.checked
          };
          saveSettingsLocal();
        });
      });
      Array.from(document.querySelectorAll("[data-gd-scroll]")).forEach((node) => {
        node.addEventListener("click", () => {
          const id = String(node.getAttribute("data-gd-scroll") || "");
          const target = id ? document.getElementById(id) : null;
          if (target) target.scrollIntoView({ behavior: "smooth", block: "start" });
        });
      });
      if (el.adolScanBtn) {
        el.adolScanBtn.addEventListener("click", async () => {
          await runAdolScan();
        });
      }
      if (el.adolRefreshBtn) {
        el.adolRefreshBtn.addEventListener("click", async () => {
          await refreshAdolStatus();
        });
      }
      if (el.adolWatchlistInput) {
        el.adolWatchlistInput.addEventListener("blur", () => {
          app.adol.watchlistText = String(el.adolWatchlistInput.value || "");
          saveSettingsLocal();
        });
      }
      ...
      if (el.loginBtn) {
        el.loginBtn.addEventListener("click", async () => {
          await window.forcePlatformLogin();
        });
      }
      if (el.loginEmail) {
        el.loginEmail.addEventListener("keydown", async (e) => {
          if (e.key === "Enter") {
            e.preventDefault();
            await window.forcePlatformLogin();
          }
      ...
      if (el.loginPassword) {
        el.loginPassword.addEventListener("keydown", async (e) => {
          if (e.key === "Enter") {
            e.preventDefault();
            await window.forcePlatformLogin();
          }
    ...
    function init() {
      loadSettingsLocal();
      app.adol.page = "liquidity";
      gdApplyAutoPolicyDefaults();
      saveSettingsLocal();
