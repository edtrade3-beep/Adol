    ...
    const app = {
      ...
      lastPrayerAlertKey: "",
      api: {
        marketDataBaseUrl: "http://127.0.0.1:8080/api/market",
        newsBaseUrl: "http://127.0.0.1:8080/api/news",
        aiBaseUrl: "http://127.0.0.1:8080/api/ai/report",
    ...
    function providerQuerySuffix() {
      ...
      const marketDataKey = String(app.apiKeys.marketData || "").trim();
      const finnhubKey = String(app.apiKeys.finnhub || marketDataKey).trim();
      const fmpKey = String(app.apiKeys.fmp || marketDataKey).trim();
      const tdKey = String(app.apiKeys.td || marketDataKey).trim();
      if (finnhubKey) params.set("finnhubKey", finnhubKey);
      if (fmpKey) params.set("fmpKey", fmpKey);
      if (tdKey) params.set("tdKey", tdKey);
    ...
    function setActiveTab(tabName) {
      ...
      tabPanels.forEach((panel) => panel.classList.toggle("active", panel.getAttribute("data-panel") === tabName));
    }
    ...
    function renderMacro() { el.macro.innerHTML = app.macroItems.map((m) => `<div class="tile"><div class="row"><div><div class="k">${m.name}</div><div style="font-weight:700;">${m.value}</div></div><span class="badge">${m.delta}</span></div></div>`).join(""); }
    function renderNews() { el.news.innerHTML = app.newsItems.map((n) => `<div class="news-item"><div class="badges"><span class="badge">${n.tag}</span><span class="badge">${n.impact}</span></div><div style="font-weight:700;">${n.title}</div><div style="font-size:12px;color:var(--muted2);">${n.source}</div></div>`).join(""); }
