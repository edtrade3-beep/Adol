    ...
    const ARABIC_MUSIC_QUERIES = [
      { name: "Arabic Music Mix", query: "arabic music mix" },
      { name: "Arabic Oud Instrumental", query: "arabic oud instrumental" },
      { name: "Arabic Chill / Lofi", query: "arabic lofi chill music" },
      { name: "Arabic Classics", query: "arabic classic songs" },
      { name: "Nasheed Vocals", query: "nasheed vocals" }
    ];
    ...
    const app = {
      ...
      arabicMusicQuery: ARABIC_MUSIC_QUERIES[0].query,
      quranLastFailedUrl: "",
    ...
    function saveSettingsLocal() {
      localStorage.setItem("market-ai-platform-settings", JSON.stringify({
        ...
        arabicMusicQuery: app.arabicMusicQuery,
    ...
    function loadSettingsLocal() {
      ...
      try {
        ...
        if (typeof p.arabicMusicQuery === "string" && p.arabicMusicQuery.trim()) app.arabicMusicQuery = p.arabicMusicQuery.trim();
    ...
    function buildArabicMusicEmbedUrl(query) {
      const q = String(query || "").trim() || ARABIC_MUSIC_QUERIES[0].query;
      return `https://www.youtube.com/embed?listType=search&list=${encodeURIComponent(q)}&autoplay=0&controls=1&rel=0`;
    }

    function applyArabicMusicQuery(query) {
      if (!el.arabicMusicFrame || !el.arabicMusicQuery) return;
      const pick = ARABIC_MUSIC_QUERIES.find((x) => x.query === query) || ARABIC_MUSIC_QUERIES[0];
      app.arabicMusicQuery = pick.query;
      el.arabicMusicQuery.value = pick.query;
      const nextSrc = buildArabicMusicEmbedUrl(pick.query);
    ...
    function renderArabicMusicQueries() {
      ...
      el.arabicMusicQuery.innerHTML = ARABIC_MUSIC_QUERIES.map((x) => `<option value="${x.query}">${x.name}</option>`).join("");
      applyArabicMusicQuery(app.arabicMusicQuery);
    }
...
${model.symbol} is ${model.trend.toLowerCase()} with ${model.momentum.toLowerCase()} momentum. Confirm reclaim/reject behavior at ${formatMoney(model.gammaFlip)} before full size.</div>
    ...
    function renderAll() { renderSummary(); renderOverview(); renderHeat(); renderWatch(); renderMacro(); renderNews(); renderLiveTv(); renderPrayerTimes(); renderMetrics(); renderChart(); renderLog(); renderWebV3(); renderConfig(); renderDeepDive(); renderProTrader(); applyTheme(); }
    ...
    function bind() {
      ...
      if (el.arabicMusicQuery) {
        el.arabicMusicQuery.addEventListener("change", () => {
          applyArabicMusicQuery(el.arabicMusicQuery.value);
          saveSettingsLocal();
